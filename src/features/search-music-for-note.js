    ...
    try{
      ...
      r.innerHTML = data.results.map((song,idx)=>`
        <div style="display:flex;align-items:center;gap:12px;padding:10px 4px;border-bottom:1px solid #111">
          <div data-preview-url="${encodeURIComponent(song.previewUrl||'')}" onclick="togglePreviewPlay(${idx}, decodeURIComponent(this.dataset.previewUrl))" id="preview-btn-${idx}" style="width:44px;height:44px;border-radius:8px;position:relative;cursor:pointer;flex-shrink:0;overflow:hidden">
            <img src="${esc(song.artworkUrl60)}" style="width:100%;height:100%;object-fit:cover">
          ...
          <div data-title="${encodeURIComponent(song.trackName||'')}" data-artist="${encodeURIComponent(song.artistName||'')}" data-artwork="${encodeURIComponent(song.artworkUrl60||"")}" data-preview="${encodeURIComponent(song.previewUrl||"")}" onclick="selectNoteMusicResult(decodeURIComponent(this.dataset.title), decodeURIComponent(this.dataset.artist), decodeURIComponent(this.dataset.artwork), decodeURIComponent(this.dataset.preview))" style="flex:1;overflow:hidden;cursor:pointer">
            <div style="font-weight:700;font-size:13px;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(song.trackName)}</div>
            <div style="font-size:11px;color:#888;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(song.artistName)}</div>
          </div>
          <div data-title="${encodeURIComponent(song.trackName||'')}" data-artist="${encodeURIComponent(song.artistName||'')}" data-artwork="${encodeURIComponent(song.artworkUrl60||"")}" data-preview="${encodeURIComponent(song.previewUrl||"")}" onclick="selectNoteMusicResult(decodeURIComponent(this.dataset.title), decodeURIComponent(this.dataset.artist), decodeURIComponent(this.dataset.artwork), decodeURIComponent(this.dataset.preview))" style="color:#E1306C;font-size:12px;font-weight:700;cursor:pointer;padding:6px 10px;flex-shrink:0">Select</div>
        </div>`).join('');
