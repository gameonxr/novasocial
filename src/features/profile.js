...
function profTab(t){
  ...
  if(t==='posts'){
    ...
    const p=window._pp||[];
    g.innerHTML=!p.length?'<div style="grid-column:1/-1;text-align:center;padding:52px;color:#333"><div style="font-size:44px;margin-bottom:10px">📷</div>Koi post nahi</div>':p.map(x=>'<div class="pitem" onclick="viewPost(\''+x.id+'\')">'+(x.media_url?'<img src="'+esc(cldUrl(x.media_url, NOVA_MEDIA_CONFIG.grid_thumb.cloudTransform))+'" loading="lazy">':'<div style="display:flex;align-items:center;justify-content:center;width:100%;height:100%;color:#333;font-size:32px">📷</div>')+'</div>').join('');
  }else if(t==='reels'){
    rb.style.borderBottomColor='#fff';rb.innerHTML=ico('film','#fff',22);
    const r=window._pr||[];
    g.innerHTML=!r.length?'<div style="grid-column:1/-1;text-align:center;padding:52px;color:#333"><div style="font-size:44px;margin-bottom:10px">🎬</div>Koi reel nahi</div>':r.map(x=>'<div class="pitem" onclick="viewPost(\''+x.id+'\')" style="position:relative">'+(x.media_url?'<img src="'+esc(cldUrl(x.media_url, NOVA_MEDIA_CONFIG.grid_thumb.cloudTransform))+'" loading="lazy">':'<div style="display:flex;align-items:center;justify-content:center;width:100%;height:100%;color:#333;font-size:32px">🎬</div>')+'<div style="position:absolute;top:6px;right:6px">'+ico('film','#fff',14)+'</div></div>').join('');
  }else if(t==='saved'){
