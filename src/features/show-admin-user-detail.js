  ...
  try {
    ...
    body.innerHTML = `<div style="padding:0">
      ...
      ${!isSelf?`<div style="padding:0 16px 16px;display:flex;flex-direction:column;gap:8px">
        ...
        ${user.is_banned?`<button data-action-username="${encodeURIComponent(user.username||'')}" onclick="adminUnbanUser('${userId}',decodeURIComponent(this.dataset.actionUsername))" style="padding:12px;background:rgba(61,184,61,0.1);border:1px solid #3db83d;border-radius:12px;color:#3db83d;font-weight:700;font-size:13px;cursor:pointer">${ico('unlock','#3db83d',16)} Unban User</button>`:`<button data-action-username="${encodeURIComponent(user.username||'')}" onclick="adminBanUser('${userId}',decodeURIComponent(this.dataset.actionUsername))" style="padding:12px;background:rgba(255,68,68,0.1);border:1px solid #ff4444;border-radius:12px;color:#ff4444;font-weight:700;font-size:13px;cursor:pointer">${ico('lock','#ff4444',16)} Ban User</button>`}
        ${user.is_msg_banned?`<button data-action-username="${encodeURIComponent(user.username||'')}" onclick="adminUnbanMsgUser('${userId}',decodeURIComponent(this.dataset.actionUsername))" style="padding:12px;background:rgba(61,184,61,0.1);border:1px solid #3db83d;border-radius:12px;color:#3db83d;font-weight:700;font-size:13px;cursor:pointer">Unban Messages</button>`:`<button data-action-username="${encodeURIComponent(user.username||'')}" onclick="adminBanMsgUser('${userId}',decodeURIComponent(this.dataset.actionUsername))" style="padding:12px;background:rgba(255,170,0,0.1);border:1px solid #ffaa00;border-radius:12px;color:#ffaa00;font-weight:700;font-size:13px;cursor:pointer">Ban Messages Only</button>`}
        ${user.is_admin?`<button data-action-username="${encodeURIComponent(user.username||'')}" onclick="adminDemoteUser('${userId}',decodeURIComponent(this.dataset.actionUsername))" style="padding:12px;background:rgba(255,170,0,0.1);border:1px solid #ffaa00;border-radius:12px;color:#ffaa00;font-weight:700;font-size:13px;cursor:pointer">${ico('crown','#ffaa00',16)} Remove Admin</button>`:`<button data-action-username="${encodeURIComponent(user.username||'')}" onclick="adminPromoteUser('${userId}',decodeURIComponent(this.dataset.actionUsername))" style="padding:12px;background:rgba(168,85,247,0.1);border:1px solid #a855f7;border-radius:12px;color:#a855f7;font-weight:700;font-size:13px;cursor:pointer">${ico('crown','#a855f7',16)} Promote to Admin</button>`}
