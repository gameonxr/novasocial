...
assert.strictEqual(count(/<script\b/gi), 214, 'index.html must retain 214 script tags');
assert.strictEqual(count(/<\/script>/gi), 214, 'every script tag must be closed');
assert.strictEqual(count(/<script\s+src=/gi), 213, '213 extracted/external script tags must remain integrated');
...
console.log('SCRIPT_TAGS=214');
console.log('SCRIPT_CLOSURES=214');
console.log('EXTERNAL_SCRIPT_TAGS=213');
