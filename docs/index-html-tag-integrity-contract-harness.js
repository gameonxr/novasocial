...
assert.strictEqual(count(/<script\b/gi), 221, 'index.html must retain 221 script tags after Notes reactor-list split');
assert.strictEqual(count(/<\/script>/gi), 221, 'every script tag must be closed after Notes reactor-list split');
assert.strictEqual(count(/<script\s+src=/gi), 220, '220 extracted/external script tags must remain integrated');
...
console.log('SCRIPT_TAGS=221');
console.log('SCRIPT_CLOSURES=221');
console.log('EXTERNAL_SCRIPT_TAGS=220');
