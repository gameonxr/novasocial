...
assert.strictEqual(count(/<script\b/gi), 235, 'index.html must retain 235 script tags after the Notes submission split');
assert.strictEqual(count(/<\/script>/gi), 235, 'every script tag must be closed after the Notes submission split');
assert.strictEqual(count(/<script\s+src=/gi), 234, '234 extracted/external script tags must remain integrated');
