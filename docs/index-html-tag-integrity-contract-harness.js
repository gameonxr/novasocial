...
assert.strictEqual(count(/<script\b/gi), 213, 'index.html must retain 213 script tags');
assert.strictEqual(count(/<\/script>/gi), 213, 'every script tag must be closed');
assert.strictEqual(count(/<script\s+src=/gi), 212, '212 extracted/external script tags must remain integrated');
...
console.log('SCRIPT_TAGS=213');
console.log('SCRIPT_CLOSURES=213');
console.log('EXTERNAL_SCRIPT_TAGS=212');
