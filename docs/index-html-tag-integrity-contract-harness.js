...
assert.strictEqual(count(/<script\b/gi), 234, 'index.html must retain 233 script tags after the DMs renderer split');
assert.strictEqual(count(/<\/script>/gi), 234, 'every script tag must be closed after the DMs renderer split');
assert.strictEqual(count(/<script\s+src=/gi), 233, '232 extracted/external script tags must remain integrated');
