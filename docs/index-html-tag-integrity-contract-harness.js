...
assert.strictEqual(count(/<script\b/gi), 220, 'index.html must retain 219 script tags after Note deletion split');
assert.strictEqual(count(/<\/script>/gi), 220, 'every script tag must be closed after Note deletion split');
assert.strictEqual(count(/<script\s+src=/gi), 219, '218 extracted/external script tags must remain integrated');
...
console.log('SCRIPT_TAGS=220');
console.log('SCRIPT_CLOSURES=220');
console.log('EXTERNAL_SCRIPT_TAGS=219');
