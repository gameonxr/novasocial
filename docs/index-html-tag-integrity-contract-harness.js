...
assert.strictEqual(count(/<script\b/gi), 316, 'index.html must retain 236 script tags after the Notes submission split');
assert.strictEqual(count(/<\/script>/gi), 316, 'every script tag must be closed after the Notes submission split');
assert.strictEqual(count(/<script\s+src=/gi), 315, '235 extracted/external script tags must remain integrated');
