...
assert.strictEqual(count(/<script\b/gi), 231, 'index.html must retain 231 script tags after the DMs renderer split');
assert.strictEqual(count(/<\/script>/gi), 231, 'every script tag must be closed after the DMs renderer split');
assert.strictEqual(count(/<script\s+src=/gi), 230, '229 extracted/external script tags must remain integrated');
...
console.log('SCRIPT_TAGS=231');
console.log('SCRIPT_CLOSURES=231');
