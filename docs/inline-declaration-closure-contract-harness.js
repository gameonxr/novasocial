...
assert.strictEqual(declarations.length, 245, 'inline application script must retain 245 function declarations after the four approved protected owner groups');
const remainingInlineProtectedNames = protectedNames.filter(name => !['spawnLikeParticles', 'syncLocalDeletionFallback', 'enablePushFromSettings', 'resetPushFromSettings', 'viewNote', 'removeMyNoteFromViewer'].includes(name));
