...
const trailing = scripts.slice(-5).map(item => item.src);
assert.deepStrictEqual(trailing, ['src/features/spawn-like-particles.js', 'src/features/sync-local-deletion-fallback.js', 'src/features/push-settings.js', 'src/features/note-viewer-owners.js', 'src/features/like-effects.js'], 'final five scripts must preserve required order after four protected owner groups');
