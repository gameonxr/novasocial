...
const trailing = scripts.slice(-9).map(item => item.src);
assert.deepStrictEqual(trailing, ['src/features/spawn-like-particles.js', 'src/features/sync-local-deletion-fallback.js', 'src/features/push-settings.js', 'src/features/note-reactors-list-owner.js', 'src/features/note-viewer-owners.js', 'src/features/note-deletion-owner.js', 'src/features/story-editor-owners.js', 'src/features/reels-video-windowing.js', 'src/features/like-effects.js'], 'final nine scripts must preserve required order after the Notes reactor-list split');
