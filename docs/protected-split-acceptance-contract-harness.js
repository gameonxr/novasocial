...
assert(matrix.includes('Protected production splits | 0/19 signatures moved'), 'matrix must keep the zero-split decision');
assert(matrix.includes('Contract and harness are present; browser proof is not yet established'), 'browser proof must remain explicitly unestablished');
assert(gate.includes('Direct extraction is explicitly blocked until adapter and proof work passes'), 'high-risk gate must remain blocked');
...
for (const signature of protectedSignatures) {
  ...
  assert(!source.includes(signature), `protected signature must not be extracted: ${signature}`);
}
