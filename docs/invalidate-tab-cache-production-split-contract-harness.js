...
assert.strictEqual(sourceFiles.length, 229, 'production split must retain 229 extracted JavaScript modules after the DMs renderer split');
...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 231, '230 classic script tags must remain after the DMs renderer split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 231, '230 classic script closures must remain after the DMs renderer split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 230, '230 external classic script tags must remain after the DMs renderer split');
