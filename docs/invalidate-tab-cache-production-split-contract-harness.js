...
assert.strictEqual(sourceFiles.length, 228, 'production split must retain 228 extracted JavaScript modules after the DMs renderer split');
...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 230, '230 classic script tags must remain after the DMs renderer split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 230, '230 classic script closures must remain after the DMs renderer split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 229, '229 external classic script tags must remain after the DMs renderer split');
