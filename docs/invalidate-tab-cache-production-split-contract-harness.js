...
assert.strictEqual(sourceFiles.length, 233, 'production split must retain 233 extracted JavaScript modules after the DMs renderer split');
...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 235, '233 classic script tags must remain after the Push permission banner split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 235, '233 classic script closures must remain after the Push permission banner split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 234, '232 external classic script tags must remain after the Push permission banner split');
