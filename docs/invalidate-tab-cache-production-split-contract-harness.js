...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 409, '234 classic script tags must remain after the Push permission banner split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 409, '234 classic script closures must remain after the Push permission banner split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 408, '233 external classic script tags must remain after the Push permission banner split');
