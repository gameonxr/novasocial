...
assert.strictEqual(sourceFiles.length, 227, 'production split must retain 227 extracted JavaScript modules after confirmCropPreview split');
...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 229, '229 classic script tags must remain after confirmCropPreview split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 229, '229 classic script closures must remain after confirmCropPreview split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 228, '228 external classic script tags must remain after confirmCropPreview split');
