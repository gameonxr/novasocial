...
assert.strictEqual(sourceFiles.length, 225, 'production split must retain 225 extracted JavaScript modules');
...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 227, '227 classic script tags must remain');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 227, '227 classic script closures must remain');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 226, '226 external classic script tags must remain');
