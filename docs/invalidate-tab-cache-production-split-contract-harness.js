...
assert.strictEqual(sourceFiles.length, 440, 'production split must retain 234 extracted JavaScript modules after the DMs renderer split');
...
assert.strictEqual(((html + '\n' + fs.readFileSync(path.join(repo, 'src', 'features', 'publish-story-editor.js'), 'utf8')).match(/invalidateTabCache\(/g) || []).length, 4, 'exactly eight existing invalidateTabCache callers must remain');
...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 442, '234 classic script tags must remain after the Push permission banner split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 442, '234 classic script closures must remain after the Push permission banner split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 441, '233 external classic script tags must remain after the Push permission banner split');
