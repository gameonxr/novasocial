...
assert.strictEqual(sourceFiles.length, 239, 'production split must retain 234 extracted JavaScript modules after the DMs renderer split');
...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 241, '234 classic script tags must remain after the Push permission banner split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 241, '234 classic script closures must remain after the Push permission banner split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 240, '233 external classic script tags must remain after the Push permission banner split');
