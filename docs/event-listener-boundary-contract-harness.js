...
assert.strictEqual(sourceFiles.length, 347, '234 extracted JavaScript modules must remain present after the DMs renderer split');
assert.strictEqual(count(sourceText, 'addEventListener'), 78, 'extracted modules must retain the audited 74 listener registrations after Reels windowing helper split');
...
assert.strictEqual(count(indexText, 'addEventListener'), 26, 'index.html must retain 30 listener registrations after the authorized forward-message selector');
