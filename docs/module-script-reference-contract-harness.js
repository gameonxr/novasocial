...
assert.strictEqual(modules.length, 214, 'all 214 extracted JavaScript modules must remain present after Push settings split');
...
const trailing = ['nova-init.js', 'spawn-like-particles.js', 'sync-local-deletion-fallback.js', 'push-settings.js', 'like-effects.js'].map(name => html.lastIndexOf(`<script src="src/features/${name}"></script>`));
