...
| Calls/voice | Permission denial or mocked setup, cleanup path | No live call or microphone action is required; voice permission-denied and recording start/stop browser mock evidence PASS |
...
`docs/reversible-browser-proof-contract-harness.js` verifies that the global matrix still marks full browser proof as remaining, that the protected extraction gate remains blocked, that all current seam-preparation contracts, `docs/particle-browser-proof-evidence.txt`, and `docs/particle-parity-rollback-evidence.txt`, `docs/voice-browser-proof-evidence.txt`, `docs/push-browser-proof-evidence.txt`, `docs/push-denied-browser-proof-evidence.txt`, `docs/push-granted-browser-proof-evidence.txt`, `docs/push-default-denied-browser-proof-evidence.txt`, `docs/push-default-granted-browser-proof-evidence.txt`, `docs/push-default-dismissed-browser-proof-evidence.txt`, `docs/push-request-failure-browser-proof-evidence.txt`, `docs/push-reset-failure-browser-proof-evidence.txt`, `docs/push-reset-success-browser-proof-evidence.txt`, `docs/recording-start-stop-browser-proof-evidence.txt`, `docs/notes-browser-proof-evidence.txt`, `docs/notes-music-insert-browser-proof-evidence.txt`, `docs/notes-update-failure-browser-proof-evidence.txt`, `docs/deletion-fallback-browser-proof-evidence.txt`, and `docs/deletion-fallback-valid-queue-browser-proof-evidence.txt` exist, and that protected production owners remain absent from `src/`. It reports particle mock, voice permission-denied mock, recording start/stop mock, Push unsupported-capability mock, Push denied-permission mock, Push granted/resubscribe mock, Push default-denied mock, Push default-granted mock, Push default-dismissed mock, Push request-failure mock, Push reset-failure mock, Push reset-success mock, Notes empty-validation mock, Notes music-backed insert mock, Notes update-failure mock, deletion-fallback error mock, deletion-fallback valid-queue mock, parity, and rollback evidence while intentionally keeping production-split proof **remaining**.
...
22. [`MIGRATION_MAP.md`](../MIGRATION_MAP.md)
