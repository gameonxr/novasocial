...
const expectedNames = [
  ...
  'sendCmt', 'showApp', 'showNavDebugLog', 'showNovaUniverseHub', 'spawnLikeParticles', 'syncLocalDeletionFallback', 'renderDMs', 'renderReels', 'enablePushFromSettings', 'resetPushFromSettings', 'subscribeToPushNotifications', 'forceResubscribePush', 'viewNote', 'removeMyNoteFromViewer', 'deleteMyNote', 'renderStoryElements', 'reactToNote', 'maybeShowPushPermissionBanner', 'silentPushResubscribeIfGranted', '_applyReelsVideoWindowing', 'submitNote', 'toggleLike', 'typingSub', 'setAppealsFilter', 'setReportsFilter', 'setVerifyFilter', 'toggleSVMute', 'cleanupExpiredStories', 'deleteCloudinaryMedia', 'saveAccountSession', 'removeAccountSession', 'updateLastSeen', 'cleanupExpiredStoryMedia', 'dismissIncomingCallBanner', 'createIncomingCallBanner', 'setTyping', 'loadAdminTab', 'showAdminPanel', 'destroyReelsPersistentContainer', 'invalidateAllTabCache', 'loadProf', 'resetAccountScopedUiState',
].sort();
...
assert.strictEqual(files.length, 250, 'index.html plus 240 extracted modules must be audited after the DMs renderer owner split');
assert.strictEqual(matches.length, 237, 'application surface must retain 221 explicit window assignments after the Notes submission owner split');
