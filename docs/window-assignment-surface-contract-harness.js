...
assert.strictEqual(files.length, 231, 'index.html plus 230 extracted modules must be audited after the DMs renderer owner split');
assert.strictEqual(matches.length, 217, 'application surface must retain 217 explicit window assignments after the Notes reaction owner split');
