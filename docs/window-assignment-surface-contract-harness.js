...
const expectedNames = [
  ...
  'sendCmt', 'showApp', 'showNavDebugLog', 'showNovaUniverseHub', 'spawnLikeParticles', 'syncLocalDeletionFallback', 'toggleLike', 'typingSub',
].sort();
...
assert.strictEqual(files.length, 214, 'index.html plus 213 extracted modules must be audited after the two approved splits');
assert.strictEqual(matches.length, 194, 'application surface must retain 194 explicit window assignments after the two approved splits');
