...
assert.strictEqual(files.length, 230, 'index.html plus 229 extracted modules must be audited after the DMs renderer owner split');
assert.strictEqual(matches.length, 216, 'application surface must retain 216 explicit window assignments after the DMs renderer split');
