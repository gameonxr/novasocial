...
assert.strictEqual(files.length, 390, 'index.html plus 240 extracted modules must be audited after the DMs renderer owner split');
assert.strictEqual(matches.length, 377, 'application surface must retain 221 explicit window assignments after the Notes submission owner split');
