...
The rollback proof requires rerunning the particle behavior harness, protected marker parity, browser-proof checks, and the complete Branch2 regression gate after reverting the split in a disposable local checkout or equivalent read-only tree. The published split’s production checks currently pass; this contract does not authorize moving any of the remaining 18 protected systems.
