...
assert.strictEqual(sourceFiles.length, 226, 'production split must retain 226 extracted JavaScript modules after confirmCropPreview split');
...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 228, '228 classic script tags must remain after confirmCropPreview split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 228, '228 classic script closures must remain after confirmCropPreview split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 227, '227 external classic script tags must remain after confirmCropPreview split');
