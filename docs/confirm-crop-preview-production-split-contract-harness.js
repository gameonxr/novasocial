...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 389, '234 classic script tags must remain after the DMs renderer split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 389, '234 classic script closures must remain after the DMs renderer split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 388, '234 external classic script tags must remain after the DMs renderer split');
