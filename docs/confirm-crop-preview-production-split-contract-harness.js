...
assert.strictEqual(sourceFiles.length, 230, 'production split must retain 230 extracted JavaScript modules after the DMs renderer split');
...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 232, '230 classic script tags must remain after the DMs renderer split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 232, '230 classic script closures must remain after the DMs renderer split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 231, '231 external classic script tags must remain after the DMs renderer split');
