...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 233, '233 classic script tags must remain after the DMs renderer split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 233, '233 classic script closures must remain after the DMs renderer split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 232, '232 external classic script tags must remain after the DMs renderer split');
