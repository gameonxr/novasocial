...
assert.strictEqual(sourceFiles.length, 265, 'after-split audit must include 234 extracted JavaScript modules after the DMs renderer split');
...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 267, '236 script tags must remain after the DMs renderer split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 267, '233 script closures must remain after the DMs renderer split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 266, '234 external scripts must remain after the DMs renderer split');
