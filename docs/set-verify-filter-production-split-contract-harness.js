...
assert.strictEqual(sourceFiles.length, 227, 'after-split audit must include 227 extracted JavaScript modules after confirmCropPreview split');
...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 229, '229 script tags must remain after the confirmCropPreview split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 229, '229 script closures must remain after the confirmCropPreview split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 228, '228 external scripts must remain after the confirmCropPreview split');
