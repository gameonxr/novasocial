...
assert.strictEqual(sourceFiles.length, 229, 'after-split audit must include 229 extracted JavaScript modules after the DMs renderer split');
...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 231, '231 script tags must remain after the DMs renderer split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 231, '231 script closures must remain after the DMs renderer split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 230, '230 external scripts must remain after the DMs renderer split');
