...
assert.strictEqual(sourceFiles.length, 277, 'after-split audit must include 234 extracted JavaScript modules after the DMs renderer split');
...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 279, '236 script tags must remain after the DMs renderer split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 279, '233 script closures must remain after the DMs renderer split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 278, '234 external scripts must remain after the DMs renderer split');
