...
assert.strictEqual(sourceFiles.length, 305, 'after-split audit must include 234 extracted JavaScript modules after the DMs renderer split');
...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 307, '236 script tags must remain after the DMs renderer split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 307, '233 script closures must remain after the DMs renderer split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 306, '234 external scripts must remain after the DMs renderer split');
