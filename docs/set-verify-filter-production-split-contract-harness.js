...
assert.strictEqual(sourceFiles.length, 232, 'after-split audit must include 232 extracted JavaScript modules after the DMs renderer split');
...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 234, '234 script tags must remain after the DMs renderer split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 234, '232 script closures must remain after the DMs renderer split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 233, '233 external scripts must remain after the DMs renderer split');
