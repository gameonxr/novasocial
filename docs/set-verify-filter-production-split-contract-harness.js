...
assert.strictEqual(sourceFiles.length, 226, 'after-split audit must include 226 extracted JavaScript modules after confirmCropPreview split');
...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 228, '228 script tags must remain after the confirmCropPreview split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 228, '228 script closures must remain after the confirmCropPreview split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 227, '227 external scripts must remain after the confirmCropPreview split');
