...
assert.strictEqual(sourceFiles.length, 228, 'after-split audit must include 228 extracted JavaScript modules after the DMs renderer split');
...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 230, '230 script tags must remain after the DMs renderer split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 230, '230 script closures must remain after the DMs renderer split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 229, '229 external scripts must remain after the DMs renderer split');
