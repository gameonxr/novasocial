...
assert.strictEqual(sourceFiles.length, 231, 'after-split audit must include 231 extracted JavaScript modules after the DMs renderer split');
...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 233, '233 script tags must remain after the DMs renderer split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 233, '232 script closures must remain after the DMs renderer split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 232, '232 external scripts must remain after the DMs renderer split');
