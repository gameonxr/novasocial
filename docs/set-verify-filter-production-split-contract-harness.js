...
const verifyTabModule = fs.readFileSync(path.join(repo, 'src', 'features', 'admin-tab-verify.js'), 'utf8');
...
assert.strictEqual(sourceFiles.length, 424, 'after-split audit must include 234 extracted JavaScript modules after the DMs renderer split');
...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 426, '236 script tags must remain after the DMs renderer split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 426, '233 script closures must remain after the DMs renderer split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 425, '234 external scripts must remain after the DMs renderer split');
