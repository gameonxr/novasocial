...
assert.strictEqual(sourceFiles.length, 224, 'after-split audit must include 224 extracted JavaScript modules after toggleSVMute split');
...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 226, '226 script tags must remain after the toggleSVMute split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 226, '226 script closures must remain after the toggleSVMute split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 225, '225 external scripts must remain after the toggleSVMute split');
