...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 436, '236 script tags must remain after the DMs renderer split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 436, '233 script closures must remain after the DMs renderer split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 435, '234 external scripts must remain after the DMs renderer split');
