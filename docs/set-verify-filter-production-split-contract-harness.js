...
assert.strictEqual(sourceFiles.length, 225, 'after-split audit must include 225 extracted JavaScript modules after invalidateTabCache split');
...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 227, '227 script tags must remain after the invalidateTabCache split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 227, '227 script closures must remain after the invalidateTabCache split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 226, '226 external scripts must remain after the invalidateTabCache split');
