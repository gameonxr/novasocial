...
assert.strictEqual(sourceFiles.length, 223, 'after-split audit must include 223 extracted JavaScript modules');
...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 225, '225 script tags must remain after the verification filter split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 225, '225 script closures must remain after the verification filter split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 224, '224 external scripts must remain after the verification filter split');
