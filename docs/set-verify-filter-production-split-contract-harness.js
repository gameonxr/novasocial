...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 336, '236 script tags must remain after the DMs renderer split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 336, '233 script closures must remain after the DMs renderer split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 335, '234 external scripts must remain after the DMs renderer split');
