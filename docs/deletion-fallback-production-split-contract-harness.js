...
const approved = new Set(['function spawnLikeParticles(el){', 'async function syncLocalDeletionFallback()']);
...
assert.strictEqual(count(/<script\b/gi), 215, 'after-split opening script count must be 215');
assert.strictEqual(count(/<\/script>/gi), 215, 'after-split closing script count must be 215');
assert.strictEqual(count(/<script\s+src=/gi), 214, 'after-split external script count must be 214');
