...
assert.strictEqual(count(/<script\b/gi), 220, 'after-split opening script count must be 220 after Reels windowing helper split');
assert.strictEqual(count(/<\/script>/gi), 220, 'after-split closing script count must be 220 after Reels windowing helper split');
assert.strictEqual(count(/<script\s+src=/gi), 219, 'after-split external script count must be 219 after Reels windowing helper split');
