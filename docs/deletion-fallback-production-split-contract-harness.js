...
const approved = new Set(['async function renderDMs()', 'function spawnLikeParticles(el){', 'async function syncLocalDeletionFallback()', 'async function enablePushFromSettings()', 'async function resetPushFromSettings()', 'async function viewNote(noteId){', 'async function removeMyNoteFromViewer(noteId){', 'async function deleteMyNote()', 'function renderStoryElements()', 'async function loadNoteReactorsList(']);
...
assert.strictEqual(count(/<script\b/gi), 230, 'after-split opening script count must be 230 after the DMs renderer split');
assert.strictEqual(count(/<\/script>/gi), 230, 'after-split closing script count must be 230 after the DMs renderer split');
assert.strictEqual(count(/<script\s+src=/gi), 229, 'after-split external script count must be 229 after the DMs renderer split');
