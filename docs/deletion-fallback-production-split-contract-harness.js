...
assert.strictEqual(count(/<script\b/gi), 227, 'after-split opening script count must be 227 after invalidateTabCache split');
assert.strictEqual(count(/<\/script>/gi), 227, 'after-split closing script count must be 227 after invalidateTabCache split');
assert.strictEqual(count(/<script\s+src=/gi), 226, 'after-split external script count must be 226 after invalidateTabCache split');
