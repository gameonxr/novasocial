...
const ownerModule = fs.readFileSync(path.join(repo, 'src', 'features', 'push-subscription-owner.js'), 'utf8');
const forceModule = fs.readFileSync(path.join(repo, 'src', 'features', 'push-force-resubscribe-owner.js'), 'utf8');
const ownerLinkageStart = html.indexOf('src/features/push-subscription-owner.js');
const forceLinkageStart = html.indexOf('src/features/push-force-resubscribe-owner.js');
...
assert(ownerLinkageStart >= 0 && forceLinkageStart > ownerLinkageStart, 'normal push helper must precede force helper');
assert(forceLinkageStart >= 0, 'force helper must remain available via its external module linkage');
assert(html.indexOf('src/features/push-settings.js') > forceLinkageStart, 'force helper must precede settings module');
...
const normal = ownerModule.slice(ownerModule.indexOf('window.subscribeToPushNotifications'));
const force = forceModule.slice(forceModule.indexOf('window.forceResubscribePush'));
