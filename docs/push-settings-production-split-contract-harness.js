...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 231, '231 opening script tags required after the DMs renderer split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 231, '230 closing script tags required after the DMs renderer split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 230, '230 external script tags required after the DMs renderer split');
