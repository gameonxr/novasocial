...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 228, '228 opening script tags required after confirmCropPreview split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 228, '228 closing script tags required after confirmCropPreview split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 227, '227 external script tags required after confirmCropPreview split');
