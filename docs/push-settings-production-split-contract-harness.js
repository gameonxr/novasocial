...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 232, '232 opening script tags required after the DMs renderer split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 232, '230 closing script tags required after the DMs renderer split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 231, '231 external script tags required after the DMs renderer split');
