...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 265, '236 opening script tags required after the DMs renderer split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 265, '231 closing script tags required after the DMs renderer split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 264, '234 external script tags required after the DMs renderer split');
