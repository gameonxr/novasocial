...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 225, '225 opening script tags required after verification filter split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 225, '225 closing script tags required after verification filter split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 224, '224 external script tags required after verification filter split');
