...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 329, '236 opening script tags required after the DMs renderer split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 329, '231 closing script tags required after the DMs renderer split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 328, '234 external script tags required after the DMs renderer split');
