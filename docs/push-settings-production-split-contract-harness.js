...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 286, '236 opening script tags required after the DMs renderer split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 286, '231 closing script tags required after the DMs renderer split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 285, '234 external script tags required after the DMs renderer split');
