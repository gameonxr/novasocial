...
assert.strictEqual((html.match(/<script\b/gi) || []).length, 364, '236 opening script tags required after the DMs renderer split');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 364, '231 closing script tags required after the DMs renderer split');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 363, '234 external script tags required after the DMs renderer split');
