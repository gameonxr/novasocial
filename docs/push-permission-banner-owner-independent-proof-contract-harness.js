...
assert.strictEqual(currentHtml.split('function maybeShowPushPermissionBanner()').length - 1, 1, 'one inline Push banner owner must remain');
assert(!currentHtml.includes('src/features/push-permission-banner-owner.js'), 'production Push banner owner must not be extracted');
