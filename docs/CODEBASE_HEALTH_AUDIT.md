...
## 20. Post-publication corrections (2026-09-05)

**H3 premise correction — the shared escape helper already exists.** The audit's security scan (CHECK 12) searched for `escapeHtml` by name and reported "no escapeHtml/sanitizer helper is defined anywhere." That premise was a name-pattern miss: the repository's shared HTML-escaping helper is named `esc()` and has existed in `src/core/utils.js:4-12` since `origin/main` (monolith line 1432), with exactly the semantics the fix plan prescribed (nullish → empty string, then `& < > " '` entity escaping). It is pinned by `docs/escape-helper-contract-harness.js` (which asserts exactly one shared definition and verifies the escaping behavior end-to-end) and is already in active use by the admin-side renderers (`load-admin-content.js`, `load-appeals-list.js`, `show-admin-user-detail.js`, `load-reports-list.js`) for usernames, ban reasons, captions, and appeal text.

Consequences for the recommended fix order: adding a new `function escapeHtml` would create a duplicate owner of an existing single-owner helper, so the staged wrapping series (fix 6) deliberately reuses the existing `esc()` instead of introducing a second name. Everything else in H3 stands as written — the 398 `innerHTML` assignments, the 72 interpolated sites, the ~10 highest-touch user-controlled flows, and the staged one-file-per-commit wrapping plan with a regression run after each commit. The typing/chat/pin/story/search/explore wraps therefore read `esc(...)` rather than `escapeHtml(...)`; the protective effect is identical because it is the same shared function.
