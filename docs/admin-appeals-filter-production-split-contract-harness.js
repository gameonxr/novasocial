...
assert.strictEqual((branch2Html.match(/<script\b/gi) || []).length, 231, '231 opening script tags required after the DMs renderer split');
assert.strictEqual((branch2Html.match(/<\/script>/gi) || []).length, 231, '231 closing script tags required after the DMs renderer split');
assert.strictEqual((branch2Html.match(/<script\s+src=/gi) || []).length, 230, '230 external script tags required after the DMs renderer split');
