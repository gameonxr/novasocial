...
assert.strictEqual((branch2Html.match(/<script\b/gi) || []).length, 375, '375 opening script tags required after the DMs renderer split');
assert.strictEqual((branch2Html.match(/<\/script>/gi) || []).length, 375, '234 closing script tags required after the DMs renderer split');
assert.strictEqual((branch2Html.match(/<script\s+src=/gi) || []).length, 374, '234 external script tags required after the DMs renderer split');
