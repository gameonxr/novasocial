...
assert.strictEqual((branch2Html.match(/<script\b/gi) || []).length, 247, '247 opening script tags required after the DMs renderer split');
assert.strictEqual((branch2Html.match(/<\/script>/gi) || []).length, 247, '234 closing script tags required after the DMs renderer split');
assert.strictEqual((branch2Html.match(/<script\s+src=/gi) || []).length, 246, '234 external script tags required after the DMs renderer split');
