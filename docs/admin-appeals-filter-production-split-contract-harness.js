...
assert.strictEqual((branch2Html.match(/<script\b/gi) || []).length, 230, '230 opening script tags required after the DMs renderer split');
assert.strictEqual((branch2Html.match(/<\/script>/gi) || []).length, 230, '230 closing script tags required after the DMs renderer split');
assert.strictEqual((branch2Html.match(/<script\s+src=/gi) || []).length, 229, '229 external script tags required after the DMs renderer split');
