...
assert.strictEqual((branch2Html.match(/<script\b/gi) || []).length, 424, '397 opening script tags required after the DMs renderer split');
assert.strictEqual((branch2Html.match(/<\/script>/gi) || []).length, 424, '234 closing script tags required after the DMs renderer split');
assert.strictEqual((branch2Html.match(/<script\s+src=/gi) || []).length, 423, '234 external script tags required after the DMs renderer split');
