...
assert.strictEqual((branch2Html.match(/<script\b/gi) || []).length, 245, '245 opening script tags required after the DMs renderer split');
assert.strictEqual((branch2Html.match(/<\/script>/gi) || []).length, 245, '234 closing script tags required after the DMs renderer split');
assert.strictEqual((branch2Html.match(/<script\s+src=/gi) || []).length, 244, '234 external script tags required after the DMs renderer split');
