...
assert.strictEqual((branch2Html.match(/<script\b/gi) || []).length, 226, '226 opening script tags required after toggleSVMute split');
assert.strictEqual((branch2Html.match(/<\/script>/gi) || []).length, 226, '226 closing script tags required after toggleSVMute split');
assert.strictEqual((branch2Html.match(/<script\s+src=/gi) || []).length, 225, '225 external script tags required after toggleSVMute split');
