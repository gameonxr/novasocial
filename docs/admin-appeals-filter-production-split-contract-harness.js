...
assert.strictEqual((branch2Html.match(/<script\b/gi) || []).length, 222, '222 opening script tags required after admin filter split');
assert.strictEqual((branch2Html.match(/<\/script>/gi) || []).length, 222, '222 closing script tags required after admin filter split');
assert.strictEqual((branch2Html.match(/<script\s+src=/gi) || []).length, 221, '221 external script tags required after admin filter split');
