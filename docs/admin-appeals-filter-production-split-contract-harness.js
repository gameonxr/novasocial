...
assert.strictEqual((branch2Html.match(/<script\b/gi) || []).length, 234, '233 opening script tags required after the DMs renderer split');
assert.strictEqual((branch2Html.match(/<\/script>/gi) || []).length, 234, '233 closing script tags required after the DMs renderer split');
assert.strictEqual((branch2Html.match(/<script\s+src=/gi) || []).length, 233, '233 external script tags required after the DMs renderer split');
