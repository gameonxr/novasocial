...
assert.strictEqual((branch2Html.match(/<script\b/gi) || []).length, 410, '397 opening script tags required after the DMs renderer split');
assert.strictEqual((branch2Html.match(/<\/script>/gi) || []).length, 410, '234 closing script tags required after the DMs renderer split');
assert.strictEqual((branch2Html.match(/<script\s+src=/gi) || []).length, 409, '234 external script tags required after the DMs renderer split');
