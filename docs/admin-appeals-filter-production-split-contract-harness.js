...
assert.strictEqual((branch2Html.match(/<script\b/gi) || []).length, 227, '227 opening script tags required after invalidateTabCache split');
assert.strictEqual((branch2Html.match(/<\/script>/gi) || []).length, 227, '227 closing script tags required after invalidateTabCache split');
assert.strictEqual((branch2Html.match(/<script\s+src=/gi) || []).length, 226, '226 external script tags required after invalidateTabCache split');
