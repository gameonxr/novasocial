...
assert.strictEqual(allDocs.length, 275, '275 documentation Markdown files must be published after Reels poll UI contract checkpoint');
assert.strictEqual(allHarnesses.length, 276, '276 harness files must be published after Reels poll UI contract checkpoint');
assert.strictEqual(contractFiles.length, 272, '272 standard contract documents must be published after Reels poll UI contract checkpoint');
assert.strictEqual(harnessFiles.length, 271, '271 standard contract harnesses must be published after Reels poll UI contract checkpoint');
