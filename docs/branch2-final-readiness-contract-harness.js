...
assert.strictEqual(jsFiles.length, 359, '359 extracted JavaScript modules must remain after the admin-tab-users extraction');
assert.strictEqual(cssFiles.length, 18, '18 extracted CSS stylesheets must remain');
assert.strictEqual(featureFiles.length, 348, '348 feature modules must remain after the admin-tab-users extraction');
assert.strictEqual((html.match(/<script\b/gi) || []).length, 361, 'HTML must retain 361 script tags after the admin-tab-users extraction');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 361, 'HTML script tags must remain balanced');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 360, 'HTML must retain 360 external script tags after the admin-tab-users extraction');
