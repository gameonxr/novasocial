...
assert.strictEqual(jsFiles.length, 369, '369 extracted JavaScript modules must remain after the check-emergency-lock extraction');
assert.strictEqual(cssFiles.length, 18, '18 extracted CSS stylesheets must remain');
assert.strictEqual(featureFiles.length, 358, '358 feature modules must remain after the check-emergency-lock extraction');
assert.strictEqual((html.match(/<script\b/gi) || []).length, 371, 'HTML must retain 371 script tags after the check-emergency-lock extraction');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 371, 'HTML script tags must remain balanced');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 370, 'HTML must retain 370 external script tags after the check-emergency-lock extraction');
