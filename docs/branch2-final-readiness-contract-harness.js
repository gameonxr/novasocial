...
assert.strictEqual(jsFiles.length, 286, '286 extracted JavaScript modules must remain after the delete-msg-for-me extraction');
assert.strictEqual(cssFiles.length, 18, '18 extracted CSS stylesheets must remain');
assert.strictEqual(featureFiles.length, 275, '275 feature modules must remain after the delete-msg-for-me extraction');
assert.strictEqual((html.match(/<script\b/gi) || []).length, 288, 'HTML must retain 288 script tags after the delete-msg-for-me extraction');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 288, 'HTML script tags must remain balanced');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 287, 'HTML must retain 287 external script tags after the delete-msg-for-me extraction');
