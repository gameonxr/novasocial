...
assert.strictEqual(allDocs.length, 316, '316 documentation Markdown files must be published after the silent Push resubscribe independent-proof package');
assert.strictEqual(allHarnesses.length, 312, '312 harness files must be published after the silent Push resubscribe independent-proof package');
assert.strictEqual(contractFiles.length, 308, '308 standard contract documents must be published after the silent Push resubscribe independent-proof package');
assert.strictEqual(harnessFiles.length, 307, '307 standard contract harnesses must be published after the silent Push resubscribe independent-proof package');
assert.deepStrictEqual(allDocs.filter(file => !file.endsWith('-contract.md')).sort(), ['blocking-contract-assessment.md', 'browser-smoke-baseline-2026-08-22.md', 'notes-reaction-owner-independent-authorization-addendum.md', 'notes-submission-owner-independent-authorization-addendum.md', 'protected-contract-coverage.md', 'push-permission-banner-owner-independent-authorization-addendum.md', 'push-silent-resubscribe-owner-independent-authorization-addendum.md', 'reels-renderer-navigation-independent-authorization-addendum.md'], 'nonstandard documentation exceptions must remain explicitly mapped');
