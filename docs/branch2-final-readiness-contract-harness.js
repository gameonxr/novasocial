...
assert.strictEqual(jsFiles.length, 305, '305 extracted JavaScript modules must remain after the remove-member extraction');
assert.strictEqual(cssFiles.length, 18, '18 extracted CSS stylesheets must remain');
assert.strictEqual(featureFiles.length, 294, '294 feature modules must remain after the remove-member extraction');
assert.strictEqual((html.match(/<script\b/gi) || []).length, 307, 'HTML must retain 307 script tags after the remove-member extraction');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 307, 'HTML script tags must remain balanced');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 306, 'HTML must retain 306 external script tags after the remove-member extraction');
