...
assert.strictEqual(jsFiles.length, 291, '291 extracted JavaScript modules must remain after the listen-for-group-participants extraction');
assert.strictEqual(cssFiles.length, 18, '18 extracted CSS stylesheets must remain');
assert.strictEqual(featureFiles.length, 280, '280 feature modules must remain after the listen-for-group-participants extraction');
assert.strictEqual((html.match(/<script\b/gi) || []).length, 293, 'HTML must retain 293 script tags after the listen-for-group-participants extraction');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 293, 'HTML script tags must remain balanced');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 292, 'HTML must retain 292 external script tags after the listen-for-group-participants extraction');
