...
assert.strictEqual(jsFiles.length, 257, '257 extracted JavaScript modules must remain after the swipe-start extraction');
assert.strictEqual(cssFiles.length, 18, '18 extracted CSS stylesheets must remain');
assert.strictEqual(featureFiles.length, 246, '246 feature modules must remain after the swipe-start extraction');
assert.strictEqual((html.match(/<script\b/gi) || []).length, 259, 'HTML must retain 259 script tags after the swipe-start extraction');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 259, 'HTML script tags must remain balanced');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 258, 'HTML must retain 258 external script tags after the swipe-start extraction');
