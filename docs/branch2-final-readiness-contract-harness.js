...
assert.strictEqual(jsFiles.length, 281, '281 extracted JavaScript modules must remain after the toggle-approval-system extraction');
assert.strictEqual(cssFiles.length, 18, '18 extracted CSS stylesheets must remain');
assert.strictEqual(featureFiles.length, 270, '270 feature modules must remain after the toggle-approval-system extraction');
assert.strictEqual((html.match(/<script\b/gi) || []).length, 283, 'HTML must retain 283 script tags after the toggle-approval-system extraction');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 283, 'HTML script tags must remain balanced');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 282, 'HTML must retain 282 external script tags after the toggle-approval-system extraction');
