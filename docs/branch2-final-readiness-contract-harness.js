...
assert.strictEqual(jsFiles.length, 380, '380 extracted JavaScript modules must remain after the admin-promote-mod-to-admin extraction');
assert.strictEqual(cssFiles.length, 18, '18 extracted CSS stylesheets must remain');
assert.strictEqual(featureFiles.length, 369, '369 feature modules must remain after the admin-promote-mod-to-admin extraction');
assert.strictEqual((html.match(/<script\b/gi) || []).length, 382, 'HTML must retain 382 script tags after the admin-promote-mod-to-admin extraction');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 382, 'HTML script tags must remain balanced');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 381, 'HTML must retain 381 external script tags after the admin-promote-mod-to-admin extraction');
