...
assert.strictEqual(allDocs.length, 292, '292 documentation Markdown files must be published after Story background controls contract checkpoint');
assert.strictEqual(allHarnesses.length, 293, '293 harness files must be published after Story background controls contract checkpoint');
assert.strictEqual(contractFiles.length, 289, '289 standard contract documents must be published after Story background controls contract checkpoint');
assert.strictEqual(harnessFiles.length, 288, '288 standard contract harnesses must be published after Story background controls contract checkpoint');
