...
assert.strictEqual(allDocs.length, 288, '288 documentation Markdown files must be published after video-length-options contract checkpoint');
assert.strictEqual(allHarnesses.length, 289, '289 harness files must be published after video-length-options contract checkpoint');
assert.strictEqual(contractFiles.length, 285, '285 standard contract documents must be published after video-length-options contract checkpoint');
assert.strictEqual(harnessFiles.length, 284, '284 standard contract harnesses must be published after video-length-options contract checkpoint');
