...
assert.strictEqual(jsFiles.length, 306, '306 extracted JavaScript modules must remain after the admin-ban-msg-user extraction');
assert.strictEqual(cssFiles.length, 18, '18 extracted CSS stylesheets must remain');
assert.strictEqual(featureFiles.length, 295, '295 feature modules must remain after the admin-ban-msg-user extraction');
assert.strictEqual((html.match(/<script\b/gi) || []).length, 308, 'HTML must retain 308 script tags after the admin-ban-msg-user extraction');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 308, 'HTML script tags must remain balanced');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 307, 'HTML must retain 307 external script tags after the admin-ban-msg-user extraction');
