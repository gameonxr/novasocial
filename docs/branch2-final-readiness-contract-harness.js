...
assert.strictEqual(allDocs.length, 285, '285 documentation Markdown files must be published after Account settings renderer contract checkpoint');
assert.strictEqual(allHarnesses.length, 286, '286 harness files must be published after Account settings renderer contract checkpoint');
assert.strictEqual(contractFiles.length, 282, '282 standard contract documents must be published after Account settings renderer contract checkpoint');
assert.strictEqual(harnessFiles.length, 281, '281 standard contract harnesses must be published after Account settings renderer contract checkpoint');
