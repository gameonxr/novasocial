...
assert.strictEqual(jsFiles.length, 280, '280 extracted JavaScript modules must remain after the play-ringtone extraction');
assert.strictEqual(cssFiles.length, 18, '18 extracted CSS stylesheets must remain');
assert.strictEqual(featureFiles.length, 269, '269 feature modules must remain after the play-ringtone extraction');
assert.strictEqual((html.match(/<script\b/gi) || []).length, 282, 'HTML must retain 282 script tags after the play-ringtone extraction');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 282, 'HTML script tags must remain balanced');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 281, 'HTML must retain 281 external script tags after the play-ringtone extraction');
