...
assert.strictEqual(jsFiles.length, 395, '395 extracted JavaScript modules must remain after the admin-tab-reports extraction');
assert.strictEqual(cssFiles.length, 18, '18 extracted CSS stylesheets must remain');
assert.strictEqual(featureFiles.length, 384, '384 feature modules must remain after the admin-tab-reports extraction');
assert.strictEqual((html.match(/<script\b/gi) || []).length, 397, 'HTML must retain 397 script tags after the admin-tab-reports extraction');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 397, 'HTML script tags must remain balanced');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 396, 'HTML must retain 396 external script tags after the admin-tab-reports extraction');
