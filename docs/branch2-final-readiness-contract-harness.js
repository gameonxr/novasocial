...
assert.strictEqual(jsFiles.length, 360, '360 extracted JavaScript modules must remain after the search-gc extraction');
assert.strictEqual(cssFiles.length, 18, '18 extracted CSS stylesheets must remain');
assert.strictEqual(featureFiles.length, 349, '349 feature modules must remain after the search-gc extraction');
assert.strictEqual((html.match(/<script\b/gi) || []).length, 362, 'HTML must retain 362 script tags after the search-gc extraction');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 362, 'HTML script tags must remain balanced');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 361, 'HTML must retain 361 external script tags after the search-gc extraction');
