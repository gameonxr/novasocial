...
assert.strictEqual(jsFiles.length, 340, '340 extracted JavaScript modules must remain after the swipe-end extraction');
assert.strictEqual(cssFiles.length, 18, '18 extracted CSS stylesheets must remain');
assert.strictEqual(featureFiles.length, 329, '329 feature modules must remain after the swipe-end extraction');
assert.strictEqual((html.match(/<script\b/gi) || []).length, 342, 'HTML must retain 342 script tags after the swipe-end extraction');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 342, 'HTML script tags must remain balanced');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 341, 'HTML must retain 341 external script tags after the swipe-end extraction');
