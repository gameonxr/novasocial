...
assert.strictEqual(jsFiles.length, 383, '383 extracted JavaScript modules must remain after the admin-promote-user extraction');
assert.strictEqual(cssFiles.length, 18, '18 extracted CSS stylesheets must remain');
assert.strictEqual(featureFiles.length, 372, '372 feature modules must remain after the admin-promote-user extraction');
assert.strictEqual((html.match(/<script\b/gi) || []).length, 385, 'HTML must retain 385 script tags after the admin-promote-user extraction');
assert.strictEqual((html.match(/<\/script>/gi) || []).length, 385, 'HTML script tags must remain balanced');
assert.strictEqual((html.match(/<script\s+src=/gi) || []).length, 384, 'HTML must retain 384 external script tags after the admin-promote-user extraction');
