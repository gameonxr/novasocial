...
assert.strictEqual(allDocs.length, 277, '277 documentation Markdown files must be published after segment drag-window contract checkpoint');
assert.strictEqual(allHarnesses.length, 278, '278 harness files must be published after segment drag-window contract checkpoint');
assert.strictEqual(contractFiles.length, 274, '274 standard contract documents must be published after segment drag-window contract checkpoint');
assert.strictEqual(harnessFiles.length, 273, '273 standard contract harnesses must be published after segment drag-window contract checkpoint');
