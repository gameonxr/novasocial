...
This is a **mapping-only checkpoint**. Calls/WebRTC remains inline. The existing deterministic Calls/WebRTC harness proves behavior, but it is not by itself permission to extract production code. Before a split, the project still needs an explicit adapter seam, a protected before/after marker proof, and a reversible browser smoke proof with real call behavior left unchanged.
...
`docs/calls-webrtc-seam-preparation-contract-harness.js` scans `index.html` and confirms the dependency/global/DOM/timing markers, existing Calls/WebRTC contract and harness, protected inline signatures, and zero protected signature matches in `src/`. It does not open WebRTC, access media devices, call Supabase, or move production code.
