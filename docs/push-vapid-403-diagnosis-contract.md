...
The diagnosis is therefore **not a modularization regression**. The current client public key is malformed for Web Push application-server use, or the Edge Function is configured with a malformed/different public key. Because the Edge Function source and private secret are not present in this repository, matching-pair verification requires the Supabase project owner to inspect secrets without sharing private material.

## Safe remediation boundary

Do not invent or randomly replace the public key in `index.html`, and do not commit a private VAPID key. The owner must generate or retrieve one valid P-256 VAPID key pair, set the private key and subject in the Edge Function secrets, and update the client public key with the matching uncompressed P-256 public key in Base64URL form. Existing browser subscriptions may need controlled staging reset/resubscribe after a key rotation.
