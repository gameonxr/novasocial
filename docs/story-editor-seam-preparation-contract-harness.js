...
const deleteZoneSurface = fs.readFileSync(path.join(featureDir, 'show-delete-zone.js'), 'utf8') + '\n' + fs.readFileSync(path.join(featureDir, 'hide-delete-zone.js'), 'utf8');
assert((html + '\n' + deleteZoneSurface).includes('function showDeleteZone()') && (html + '\n' + deleteZoneSurface).includes('function hideDeleteZone()'), 'delete-zone helpers must remain protected');
