...
## 2026-08-24 — `resetAccountScopedUiState()` boundary rejection
- Audited the account-transition state owner. It normalizes the next identity, invalidates render generations, clears tab caches and scroll positions, resets feed/story/note state, destroys parked Reels DOM, clears the active screen and scroll handler, and records the current account identity to prevent cross-user state restoration.
- The account, DMs, Stories, Reels, cache, navigation, and render-race contracts keep identity transitions, in-memory state invalidation, persistent media DOM teardown, and screen cleanup inline. This is a protected account-scoped lifecycle owner rather than a safe UI-only component. No production code changed and no protected boundary was weakened.
...
## 2026-08-24 — Push subscription and permission boundary rejection
- Audited the Push owners. `subscribeToPushNotifications()` reads service-worker state, obtains or creates a browser subscription, and upserts the account/device endpoint into `push_subscriptions`. `forceResubscribePush()` unsubscribes the browser binding, deletes the old database row, and creates a fresh subscription. `maybeShowPushPermissionBanner()` owns account-scoped dismissal storage, delayed permission UI, browser permission requests, and subscription triggering.
- The Push/permission/storage contracts keep browser permission state, service-worker lifecycle, subscription credentials, account identity, database upsert/delete, local dismissal persistence, and delayed UI inline. These are protected permission/subscription boundaries rather than safe UI-only owners. No production code changed and no protected boundary was weakened.
## 2026-08-24 — silent Push resubscribe boundary rejection
- Audited the silent resubscribe owner. It checks service-worker/Push support and granted browser permission, delays execution to avoid competing with initial rendering, rechecks the active account identity, and invokes the database-backed subscription helper.
- The Push, account, permission, and timer contracts keep silent resubscription, permission gating, account identity, delayed scheduling, service-worker state, and subscription persistence inline. This is a protected Push lifecycle boundary rather than a safe UI-only owner. No production code changed and no protected boundary was weakened.
## 2026-08-24 — `_tryRestoreFromCache()` boundary rejection
- Audited the cache restoration owner. It restores the persistent Reels container and video position, resets overflow/transform state, applies Reels windowing and playback, restores cached tab HTML, manages cache age, and restores tab/DM scroll positions through deferred animation frames.
- The cache, DMs, Reels, navigation, media, and render-race contracts keep persistent media DOM, scroll restoration, cached HTML, stale-age policy, in-place refresh assumptions, and animation-frame ordering inline. This is the protected cross-feature cache restoration owner rather than a safe UI-only component. No production code changed and no protected boundary was weakened.
...
## 2026-08-24 — upload and Cloudinary transport boundary rejection
- Audited the media upload owners. `upload()` classifies post/reel/story/avatar/cover/chat/group/sticker media, performs image/video compression with UI progress feedback, and delegates to `_uploadToCloudinary()`. The transport owner selects a Cloudinary account and preset, derives an identity-scoped filename, builds the multipart payload, performs the network upload with progress callbacks, and handles account retry/error behavior.
- The media, storage, account, network, upload, and post/story/chat contracts keep compression, progress UI, active identity, Cloudinary account selection, multipart transport, retry, and error semantics inline. These are protected media/storage/network boundaries rather than safe UI-only owners. No production code changed and no protected boundary was weakened.
...
## 2026-08-24 — production media deletion boundary rejection
- Audited the production deletion owners. `deleteMediaProduction()` looks up unused media tokens, performs token-based Cloudinary deletion, marks tokens used, queues fallback deletion with source/reason/requesting identity, and falls back to local storage when database queueing fails. `deleteConversationMedia()` queries conversation messages, filters Cloudinary media URLs, and queues batch deletion before conversation removal.
- The media, storage, database, account, deletion-fallback, and DMs contracts keep destructive transport, token state, queue semantics, local fallback, identity attribution, and conversation cleanup inline. These are protected destructive media boundaries rather than safe UI-only owners. No production code changed and no protected boundary was weakened.
## 2026-08-24 — instant Cloudinary deletion transport boundary rejection
- Audited the token-based deletion transport. `_instantCloudinaryDelete()` sends the delete token to Cloudinary over a POST request, handles HTTP/JSON success semantics, and converts transport/API failures into a safe false result for the caller’s fallback path.
- The deletion-fallback, storage, network, and destructive-media contracts keep remote delete-token transport, response interpretation, and fallback control flow inline. This is a protected destructive network boundary rather than a safe UI-only owner. No production code changed and no protected boundary was weakened.
## 2026-08-24 — Story Editor renderer boundary confirmation
- Audited the Story Editor owner. It constructs the full editor DOM and tool panels, appends the editor to the document, sizes and initializes the drawing canvas, resets shared editor media/elements/undo state, and wires close-friends, brush, text, draw, sticker, music, background, and addon controls.
- The Story Editor lifecycle, canvas, media, submission, close-friends, and cleanup contracts keep editor DOM construction, shared state reset, drawing handlers, and tool-panel ownership inline. This is a protected full-editor lifecycle boundary rather than a safe UI-only component. No production code changed and no protected boundary was weakened.
## 2026-08-24 — Story Editor media and publish boundary rejection
- Audited the Story Editor close/media/publish owners. `closeStoryEditor()` tears down editor DOM and shared state; `pickStoryMedia()` creates object URLs and mutates editor media DOM; `undoStoryEditor()` mutates drawing/elements and rerenders; `publishStoryEditor()` composites image/text/canvas content, uploads video or generated images, serializes interactive overlays, preserves close-friends behavior, and continues into Story database submission and cleanup.
- The Story Editor, object-URL, media upload, Story submission, overlay, close-friends, and cleanup contracts keep editor media lifecycle, canvas rendering, upload selection, interactive overlay serialization, database submission, and teardown inline. These are protected full-lifecycle boundaries rather than safe UI-only owners. No production code changed and no protected boundary was weakened.
## 2026-08-24 — user report, mute, and block boundary rejection
- Audited the user-safety owners. `submitReport()` inserts authenticated reports with target/reason/status and handles schema/RLS failures. `muteUser()`/`unmuteUser()` persist mute state and update controls only after database success. `isMessagingBlocked()` checks both block directions, while `blockUser()` confirms intent, writes the block, auto-unfollows both directions, removes the user’s likes/comments from the current user’s posts, and handles duplicate/RLS failures; `unblockUser()` removes the block.
- The moderation, account, DMs, database, relationship, engagement, and error-policy contracts keep reporting, mute state, bidirectional block enforcement, follow cleanup, engagement deletion, identity attribution, and UI synchronization inline. These are protected account/moderation boundaries rather than safe UI-only owners. No production code changed and no protected boundary was weakened.
## 2026-08-24 — follow relationship boundary rejection
- Audited `toggleFollowProfile()`. It performs optimistic account/profile UI updates, queues offline follow actions, inserts or deletes authenticated follow rows, sends follow notifications, refreshes counts, invalidates profile/home caches, and rolls back UI/count state on failure.
- The relationship, account, offline-queue, notification, cache, and error-recovery contracts keep follow persistence, optimistic state, replay semantics, notification ordering, count updates, and rollback inline. This is a protected account/social-graph boundary rather than a safe UI-only owner. No production code changed and no protected boundary was weakened.
## 2026-08-24 — Reels/Notes toggle and feed boundary rejection
- Audited the cross-feature toggle owners. `switchReelsView()` preserves or pauses persistent Reels video state, creates the Notes container and pagination listener, restores toggle visibility, and starts Notes loading. `loadNotesFeed()` queries expiring Notes with profiles, maintains loading/offset/has-more/seen-user state, deduplicates users across pages, renders empty/error states, and recursively advances through duplicate-only pages.
- The Reels, Notes, media, database/RLS, pagination, scroll-listener, and state-lifecycle contracts keep persistent video preservation, Notes visibility, page progression, deduplication, listener ownership, and failure handling inline. These are protected cross-feature Reels/Notes boundaries rather than safe UI-only owners. No production code changed and no protected boundary was weakened.
## 2026-08-24 — group-info and chat-settings boundary rejection
- Audited `showGroupInfo()` and its adjacent settings owners. The surface reads conversation members, admin state, group settings, mute status, shared media, and follows; renders member/admin controls, invite links, mute/theme/approval settings, and leave/report actions; and delegates account/member mutations through `muteGroup()`, `toggleApprovalSystem()`, `setChatTheme()`, and group-management handlers.
- The DMs/group, admin authorization, realtime/chat, account, database, media, moderation, and settings contracts keep member identity, admin controls, group mutations, mute/theme/approval persistence, shared-media rendering, invite/leave/report flows, and UI synchronization inline. This is a protected group-chat/admin boundary rather than a safe UI-only owner. No production code changed and no protected boundary was weakened.
## 2026-08-24 — post/reel creation and group media-admin boundary rejection
- Audited `submitCreate()` and adjacent group media/admin owners. `submitCreate()` validates and trims reel media, composites image filters, uploads media, persists scheduled-post state, inserts posts with collaboration/thumbnail fallback, sends co-author/mention/follower notifications, extracts hashtags, invalidates caches, resets Reels persistence, and routes to the new surface. `uploadGCAvatar()` reads and updates group media, refreshes chat UI, and queues old Cloudinary cleanup; `saveGCName()` and admin/member handlers mutate group settings and membership state.
- The post/reel, media/storage, scheduler, database, notification, cache, Reels, group-chat, and admin contracts keep media preparation, upload ownership, scheduled state, post persistence, collaboration/notification ordering, destructive avatar cleanup, group identity, and authorization inline. These are protected creation and group-admin boundaries rather than safe UI-only owners. No production code changed and no protected boundary was weakened.
## 2026-08-24 — Reels poll UI contract checkpoint
- Added a detached contract and VM harness for the already extracted `src/features/reel-poll.js` helper. The harness verifies modal construction, existing reel-id callback wiring, missing-question and missing-option validation, successful feedback, modal closure, and zero database, network, storage, account, media-device, and navigation ownership.
- The protected `renderReels` renderer, swipe/navigation system, playback state, poll persistence decisions, and account-scoped behavior remain unchanged. This is test-only evidence; no production code or protected boundary changed.
- Focused Reels seam, protected-inline parity, contract-artifact pairing, syntax, and diff checks passed. Full regression remains required before publication.
## 2026-08-24 — Smart Replies classifier contract checkpoint
- Added a detached contract and VM harness for the pure `getSmartReplies(lastMsg)` classifier. The harness verifies case-insensitive category matching, first-rule precedence, exact suggestion lists, missing/unknown fallback behavior, and zero DOM, database, network, storage, account, media, navigation, and message side effects.
- The adjacent `showSmartReplies()` renderer and `quickSendReply()` sender remain outside this contract; `quickSendReply()` calls the protected `sendMsg` chat action. DM rendering, realtime state, and message sending remain unchanged. This is test-only evidence; no production code or protected boundary changed.
- Focused Smart Replies, DM preparation, inline-handler, protected-parity, syntax, and whitespace checks passed. Full regression remains required before publication.
## 2026-08-24 — Segment drag-window contract checkpoint
- Added a detached contract and VM harness for the already extracted `src/features/setup-segment-drag-window.js` helper. The harness verifies missing-node tolerance, zero-position initialization, 8-second/30-second bounds, midpoint and extreme touch drags, start-time rounding, label updates, cursor lifecycle, default prevention, and seeking only an already-playing local audio preview.
- Music search, preview controls, segment confirmation, Note submission, persistence, and protected DM/Reels/Calls systems remain outside this contract. This is test-only evidence; no production code or protected boundary changed.
- Focused segment-picker, music-picker, preview, contract-pairing, protected-parity, syntax, and whitespace checks passed. Full regression remains required before publication.
## 2026-08-24 — Settings Features renderer contract checkpoint
- Added a detached contract and VM harness for the already extracted `src/features/settings-features.js` renderer. The harness verifies Features modal setup, all eleven current rows and labels, icon/order preservation, delegated launcher markup, read-only `ME.id` interpolation for Story Highlights, and zero execution of delegated actions or protected side effects.
- Settings privacy/account state, Story Highlights, Smart Feed, Wallet, media tools, navigation, database/network behavior, and protected feature owners remain delegated and unchanged. This is test-only evidence; no production code or protected boundary changed.
- Focused Settings Features, AI context, DM preparation, inline-handler, protected-parity, syntax, and whitespace checks passed. Full regression remains required before publication.
## 2026-08-24 — Settings Privacy renderer contract checkpoint
- Added a detached contract and VM harness for the already extracted `src/features/settings-privacy.js` renderer. The harness verifies Privacy modal setup, all seven current rows, eleven icon requests including chevrons, enabled/disabled status rendering from read-only `PROF` flags, default-enabled read receipts, `ME.id` interpolation, callback wiring, and zero delegated action execution.
- Ghost Mode, Read Receipts, Private Account, blocked-user, Close Friends, Story Privacy, message privacy, account persistence, and protected settings/DM systems remain delegated or protected and unchanged. This is test-only evidence; no production code or protected boundary changed.
- Focused Privacy, DM preparation, contract-pairing, protected-parity, syntax, and whitespace checks passed. Full regression remains required before publication.
## 2026-08-24 — Tab-cache invalidation contract checkpoint
- Added a detached contract and VM harness for the already extracted `src/features/invalidate-tab-cache-owner.js` owner. The harness verifies classic-global availability, exact requested-key deletion, preservation of unrelated entries, missing-key tolerance, HTML linkage, and zero database, network, storage, account, DOM, media, messaging, navigation, permission, or external-service ownership.
- Cache restoration, cache reads, tab rendering, account/session state, navigation decisions, and protected feature systems remain unchanged. This is test-only evidence; no production code or protected boundary changed.
- Focused invalidation, existing tab-cache, contract-pairing, protected-parity, syntax, and whitespace checks passed. Full regression remains required before publication.
## 2026-08-24 — Like-effects wrapper contract checkpoint
- Added a detached contract and VM harness for the already extracted `src/features/like-effects.js` wrapper/listener. The harness verifies protected `toggleLike` delegation with arguments and receiver preserved, particle spawning only on a false-to-true like transition, no particles on unlike, theme-panel outside-click cleanup, HTML linkage, and zero persistence or protected action execution.
- The original like/unlike owner, reaction persistence, account state, post rendering, particle implementation, and protected application systems remain delegated or inline and unchanged. This is test-only evidence; no production code or protected boundary changed.
- Focused like-effects, post-count, contract-pairing, protected-parity, syntax, and whitespace checks passed. Full regression remains required before publication.
## 2026-08-24 — Disappearing-options renderer contract checkpoint
- Added a detached contract and VM harness for only the read-only `showDisappearingOptions(cid)` renderer slice in `src/features/disappearing.js`. The harness verifies modal setup, explanatory copy, all seven option labels/values/icons, conversation-id callback interpolation, exactly seven delegated handlers, and zero protected effects.
- The `setDisappearing(cid, val)` database mutator, conversation persistence, toasts, modal close behavior, account/chat state, and protected DM system remain explicitly excluded and unchanged. This is test-only evidence; no production code or protected boundary changed.
- Focused renderer, DM preparation, contract-pairing, protected-parity, syntax, and whitespace checks passed. Full regression remains required before publication.
## 2026-08-24 — Add-members renderer contract checkpoint
- Added a detached contract and VM harness for the already isolated `showAddMembers(cid)` modal renderer. The harness verifies global availability, Add Members modal setup, search icon request, placeholder, input/result ids, conversation-id interpolation in the delegated `searchAddMembers` callback, and zero protected member-search/member-action execution.
- The protected `searchAddMembers`, `addMemberToGroup`, conversation membership mutations, DMs realtime, message loading, block enforcement, scroll restoration, and group-chat state remain delegated or inline and unchanged. This is test-only evidence; no production code or protected boundary changed.
- Focused renderer, DM preparation, contract-pairing, protected-parity, syntax, and whitespace checks passed. Full regression remains required before publication.
## 2026-08-24 — New-DM renderer contract checkpoint
- Added a detached contract and VM harness for the already isolated `showNewDM()` modal renderer. The harness verifies global availability, New Message modal setup, search icon request, username-search placeholder, input/results ids, delegated `searchDM(this.value)` callback wiring, and zero protected DM search/chat action execution.
- The protected `searchDM`, profile/account lookup, `openChat`, message loading, realtime, block enforcement, navigation, and messaging actions remain delegated or inline and unchanged. This is test-only evidence; no production code or protected boundary changed.
- Focused renderer, DM preparation, contract-pairing, protected-parity, syntax, and whitespace checks passed. Full regression remains required before publication.
## 2026-08-24 — Account-settings renderer contract checkpoint
- Added a detached contract and VM harness for the already isolated `showSettingsAccount()` modal renderer. The harness verifies global availability, Account modal setup, six row labels/copy/order, twelve icon requests, delegated launcher wiring, and zero protected account-action execution.
- The delegated `showEditProfile`, `showPasswordReset`, `showVerificationApply`, `showAccountInfo`, `downloadMyData`, and `showDeleteAccount` actions remain outside the contract because they may access account data, security state, export data, or destructive account flows. Account/session state, database/network access, and live navigation remain unchanged. This is test-only evidence; no production code or protected boundary changed.
- Focused renderer, settings, contract-pairing, protected-parity, syntax, and whitespace checks passed. Full regression remains required before publication.
## 2026-08-24 — Group-call selection toggle contract checkpoint
- Added a detached contract and VM harness for the already isolated `togGC(uid, el)` participant-selection helper. The harness verifies global availability, in-memory `_gcs` add/remove ordering, selected and unselected checkbox styling, duplicate prevention through toggling, and missing-node tolerance.
- Group-call participant lookup, account validation, database/session state, signaling, media permissions, peer connections, call initiation, realtime channels, and teardown remain outside this contract and unchanged. This is test-only evidence; no production code or protected call behavior changed.
- Focused selection, Calls/WebRTC preparation, contract-pairing, protected-parity, syntax, and whitespace checks passed. Full regression remains required before publication.
## 2026-08-25 — Group-call setup renderer contract checkpoint
- Added a detached contract and VM harness for the already isolated `showGC()` setup-modal renderer. The harness verifies modal setup, local `_gcs` reset, group-name/member-search fields, result container, icon request, callback wiring, and zero invocation of protected search or group-creation actions.
- Group membership lookup or mutation, account validation, database/session state, signaling, media permissions, peer connections, call initiation, realtime channels, and teardown remain outside this contract and unchanged. This is test-only evidence; no production code or protected call behavior changed.
- Focused group-call renderer/selection, Calls/WebRTC preparation, contract-pairing, protected-parity, syntax, and whitespace checks passed. Full regression remains required before publication.
## 2026-08-25 — Video-length options renderer contract checkpoint
- Added a detached contract and VM harness for the already isolated `showVideoLengthOptions(dur)` preset renderer. The harness verifies numeric preset filtering, full-length markup for allowed durations, local `_videoTrimTo` defaults, long-duration toast/deferred `selectVideoLen(180)` delegation, picker display state, and missing-container no-op behavior.
- Media trimming, preview playback, file handling, upload, persistence, account state, network/database access, and protected creation behavior remain outside this contract and unchanged. This is test-only evidence; no production code or protected media behavior changed.
- Focused video-length, selection, segment-window, music-segment, cancellation, contract-pairing, protected-parity, syntax, and whitespace checks passed. Full regression remains required before publication.
## 2026-08-25 — Story draw-controls contract checkpoint
- Added a detached contract and VM harness for the already isolated `seOpenDrawTool()`, `seCloseDrawPanel()`, and `seSelectDrawType(type)` local controls. The harness verifies draw-panel display, draw-mode and canvas pointer-events transitions, selected/inactive draw-type styling, missing-panel behavior, and zero canvas-history or external side effects.
- Canvas drawing, undo history, bitmap mutation, media handling, upload, persistence, account state, network/database access, and Story publishing remain outside this contract and unchanged. This is test-only evidence; no production code or protected Story behavior changed.
- Focused draw-controls, Story-editor seam/browser parity, Stories preparation, contract-pairing, protected-parity, syntax, and whitespace checks passed. Full regression remains required before publication.
## 2026-08-25 — Story draw-color control evidence extension
- Extended the existing Story draw-controls contract and detached VM harness to cover `seSelectDrawColor(c)`. The harness verifies local color assignment, inactive color-option reset, active event-target border styling, and zero canvas-history or external side effects.
- This is an evidence-only extension with no production source change, no new artifact pair, and no change to protected Story publishing, media, or canvas behavior.
- Extended draw-controls, Story-editor seam/browser parity, Stories preparation, contract-pairing, protected-parity, syntax, and whitespace checks passed. Full regression remains required before publication.
## 2026-08-25 — Story text-controls contract checkpoint
- Added a detached contract and VM harness for the already isolated `seSelectTextColor(c)`, `seSelectFont(idx)`, and `seToggleGradientText()` local controls. The harness verifies color assignment and gradient reset, active/inactive color borders, font assignment and option styling, both gradient toggle directions, toast behavior, and zero text-persistence or external side effects.
- Text-element creation or editing, canvas rendering, media handling, upload, persistence, account state, network/database access, and Story publishing remain outside this contract and unchanged. This is test-only evidence; no production code or protected Story behavior changed.
- Focused text/draw controls, Story-editor seam/browser parity, Stories preparation, contract-pairing, protected-parity, syntax, and whitespace checks passed. Full regression remains required before publication.
## 2026-08-25 — Story text-panel controls contract checkpoint
- Added a detached contract and VM harness for the already isolated `seOpenTextTool()` and `seCloseTextPanel()` local controls. The harness verifies panel display transitions, input focus, new-entry `seEditingTextId` reset, close behavior, and zero text-element or external side effects.
- Text-element creation or editing, canvas rendering, media handling, upload, persistence, account state, network/database access, and Story publishing remain outside this contract and unchanged. This is test-only evidence; no production code or protected Story behavior changed.
- Focused text-panel/text/draw controls, Story-editor seam/browser parity, Stories preparation, contract-pairing, protected-parity, syntax, and whitespace checks passed. Full regression remains required before publication.
## 2026-08-25 — Story background controls contract checkpoint
- Added a detached contract and VM harness for the already isolated `seOpenBgTool()`, `seCloseBgPanel()`, and `seSelectBg(gradient, idx)` local controls. The harness verifies panel display, local gradient assignment, overlay styling, selected/inactive background borders, automatic close after selection, and zero persistence or external side effects.
- Story-element editing, canvas rendering, media handling, upload, persistence, account state, network/database access, and Story publishing remain outside this contract and unchanged. This is test-only evidence; no production code or protected Story behavior changed.
- Focused background/text/draw controls, Story-editor seam/browser parity, Stories preparation, contract-pairing, protected-parity, syntax, and whitespace checks passed. Full regression remains required before publication.
## 2026-08-25 — Story music controls contract checkpoint
- Added a detached contract and VM harness for the already isolated `seOpenMusicTool()`, `seCloseMusicPanel()`, `seSelectMusic(idx)`, and `removeStoryMusic()` local controls. The harness verifies picker display, static song selection, local metadata assignment, music-bar/info updates, picker close, removal cleanup, toast behavior, and zero audio or external side effects.
- Audio playback, media capture, file handling, upload, persistence, account state, network/database access, and Story publishing remain outside this contract and unchanged. This is test-only evidence; no production code or protected Story behavior changed.
- Focused music/background/text/draw controls, Story-editor seam/browser parity, Stories preparation, contract-pairing, protected-parity, syntax, and whitespace checks passed. Full regression remains required before publication.

## 2026-08-25 — Forward-message product decision and detached proof
- Added `docs/forward-message-product-decision-contract.md` and `docs/forward-message-product-decision-contract-harness.js` for the unresolved `forwardMessage` caller. The conservative scope is one existing eligible conversation, one new message, allowlisted text/media/shared-post fields, no copied source metadata, no new conversation creation, no upload, no client broadcast, non-optimistic success, and failure without compensating records.
- The harness is detached and synthetic-only. It covers success, blocked destination, unavailable destination, insert failure, payload allowlisting, source immutability, ordering, rollback behavior, and zero upload/realtime/navigation/live-mutation side effects. It does not define `window.forwardMessage` and does not change production DM behavior.
- Updated final-readiness counts to 294 Markdown docs, 295 harnesses, 291 standard contracts, and 290 standard contract harnesses. Protected accounting remains 19 signatures, 9 approved extracted owners, and 10 blocked systems; production extraction remains 0.
- Focused detached decision proof and existing caller-parity proof passed. Full regression is required after publication.

## 2026-08-25 — Authorized protected-DM forward implementation
- Implemented `forwardMessage(sourceMessageId)` and `completeForwardMessage(destinationConversationId)` inline in the protected classic application script on Branch2. The handler reuses existing conversation membership and messaging-block policy, excludes the source conversation and group conversations, allowlists text/media/shared-post fields, performs one non-optimistic insert, closes/shows success only after confirmation, and keeps the selector open on failure.
- No production module extraction, schema migration, media upload, client-side realtime broadcast, live navigation, or account mutation was added. The original `origin/main` caller-only state remains immutable; Branch2 production behavior is authorized only within the published product-decision contract.
- Added detached production-parity coverage for selector filtering, exact payload, block rejection, insert failure, source metadata isolation, and zero upload/realtime/navigation/live side effects. Focused VM proof, syntax, protected accounting, and full regression are required before publication.
...
- Updated the local HTML asset-reference contract to 248 unique local references with zero missing files, including the new `src/features/jump-to-message-owner.js` linkage.
- Updated source-boundary hygiene to 245 extracted JS/CSS files: 227 JavaScript and 18 CSS files, with zero NUL bytes, CRLF lines, or executable HTML-boundary tags.

## 2026-08-25 — Protected-system preparation and authorization package
- Added ten separate `*-protected-readiness-contract.md` and `*-protected-readiness-contract-harness.js` pairs for DM/chat/realtime, Reels renderer/navigation, Calls/WebRTC/group calls, Stories lifecycle/editor/viewer, voice recording/delivery, Push permission/resubscribe, Notes submission/reactions, account/bootstrap/security, creation/upload/media deletion, and moderation/admin.
- Each dossier separately records its dependency map, exact before/after parity boundary, detached/browser-safe proof plan, rollback artifact requirement, explicit feature-authorization requirement, and `PRODUCTION_DECISION=BLOCKED` / `PRODUCTION_CHANGE=0` status. The package does not authenticate, request permissions, upload, send, mutate, or split protected production code.
- All ten dossier harnesses pass against current source markers; protected accounting remains exactly 19 signatures, 9 approved extracted owners, and 10 blocked systems. Artifact inventory is now 306 Markdown docs, 307 harnesses, 303 standard contracts, and 302 standard contract harnesses.

## 2026-08-25 — Reels renderer/navigation independent authorization and proof gates
- Selected the bounded inline `async function renderReels()` candidate (`index.html:4321–4682`) as the safest remaining protected system for evidence-only authorization; broad renderer, swipe lifecycle, navigation stack, playback, overlays, Notes coupling, and cleanup extraction remain excluded.
- Added a scoped authorization addendum granting independent-proof work only. It pins exact immutable-origin owner parity at SHA-256 `db1f15428a4452e72b4a5223a5050151cb5deea0092b3a858afbd012b09653ca`, authorizes detached synthetic restore/empty/query-failure scenarios, and keeps `PRODUCTION_DECISION=BLOCKED` with `PRODUCTION_CHANGE=0`.
- Added an independent detached VM proof harness. It passed exact owner parity, persistent-container restore, empty read result, query failure, and zero database/network/storage/account/upload/permission/live-navigation/real-media side effects. The harness performs no real login, browser action, media device access, or live mutation.
- Added a pinned rollback artifact for baseline `15cbec0d69774ed2b35b8d0d808871204f225195`, immutable `origin/main` `ef418007c9b9a797488b4825be5f0c807da22369`, exact inline boundary, owner hash, and reversible future-experiment procedure. No production split or script-order change was made; protected accounting remains exactly 19 signatures, 9 approved extracted owners, and 10 blocked systems.

## 2026-08-25 — Reels lifecycle before/after and rollback evidence completed
- Expanded the dedicated Reels independent-proof harness to compare immutable-origin and Branch2 owners across restore, empty, query-failure, fast forward swipe, cancelled swipe, last-item edge, backward swipe, playback, progress timing, overlay wiring, navigation-stack isolation, transition timing, and cleanup.
- All requested detached gates passed: `SWIPE_BEFORE_AFTER=PASS`, `PLAYBACK_BEFORE_AFTER=PASS`, `NAVIGATION_STACK_ISOLATION_BEFORE_AFTER=PASS`, `OVERLAY_WIRING_BEFORE_AFTER=PASS`, `TIMING_BEFORE_AFTER=PASS`, and `CLEANUP_BEFORE_AFTER=PASS`. Before/after trace hashes match exactly: `608f6749f98370cd116ae6b28d78587af324f656e0c8b8fe2cfe803b69c5f751`.
- Completed a linkage-only synthetic split followed by exact rollback. Baseline and restored whole-file hashes match at `ac01d133892c21b01a0565509e2847f443190016da359fbb0392b3ce65ca5195`, and restored owner hash matches `db1f15428a4452e72b4a5223a5050151cb5deea0092b3a858afbd012b09653ca`.
- No production split, script-order change, live browser action, real media playback, permission request, network/storage/account mutation, or protected-accounting change occurred. Reels production extraction remains explicitly blocked pending separate formal approval and a real controlled non-production post-split/rollback run.

## 2026-08-25 — Reels non-production extraction candidate validated
- Prepared a temporary `/tmp` extraction candidate for the exact inline `renderReels()` owner. The candidate uses one classic `<script src="src/features/reels-renderer-experimental.js"></script>` linkage immediately before the inline application script and exposes `window.renderReels = async function(){...}` without `type="module"`, `defer`, or production file changes.
- The candidate passed exact immutable-origin owner parity, inline-owner removal, classic global exposure, script order, candidate lifecycle parity, and the existing restore/empty/failure/swipe/playback/overlay/timing/cleanup proof. Candidate module SHA-256 is `3b7985ad1d8163a186a757cdd65ca8db80f4fdb3a9054abfabbef8e5a0955a24`; candidate synthetic after-HTML SHA-256 is `16388e916adbcedbbadf0477b290e2b937fbceda1430cd4160639b8b02e62d5f`.
- The candidate was detached and temporary only. Branch2 production source remains unchanged, no live browser/media/account/network/storage action occurred, and production extraction remains `BLOCKED` pending formal approval and a controlled post-split non-production run.

## 2026-08-25 — Reels actual isolated post-split validation and rollback
- Applied the explicitly authorized bounded extraction only inside `/tmp/novasocial-reels-validation`. The temporary tree removed the inline `renderReels()` owner, added one classic `window.renderReels` owner module, and inserted one linkage before the inline application script; Branch2 production files remained unchanged.
- The actual isolated post-split tree passed exact owner/module parity, VM global exposure, classic script compatibility, script order, all detached lifecycle gates, and zero repository production changes. Post-split HTML SHA-256 was `9438d2b5b3871105db852d142e1a21d2e5db03569f1237b8bbab05aa4a00380a`; module SHA-256 was `3b7985ad1d8163a186a757cdd65ca8db80f4fdb3a9054abfabbef8e5a0955a24`.
- Executed rollback in the isolated tree. The experimental module and linkage were removed, the restored HTML SHA-256 matched the Branch2 baseline `a5179f6c626cae5936da608b268d5ac8e22965699a1f5d211cf5aa4d1c3890cc`, and `ISOLATED_ROLLBACK=PASS`. No production rollback or live browser action was performed.

## 2026-08-25 — Notes reaction owner independent authorization and proof
- Selected the bounded inline `function reactToNote(noteId, emoji, clickedEl)` owner (`index.html:10703–10725`) as the next safest protected candidate after Reels. Note creation/update, editor state, attachments, visibility, expiry, viewer audio, reactor lists, realtime setup, and navigation remain excluded.
- Added an exact scoped authorization addendum and pinned rollback artifact for detached proof only. Immutable-origin and Branch2 owner parity passed with SHA-256 `3877a5e2a75c4b95ff0e6494a03e7cb9eb2b0f02fee6fe64225c642078986880`; production extraction remains blocked and protected accounting remains unchanged.
- Added an independent detached VM harness covering successful clicked-element reaction, successful reaction without a clicked element, database-error handling, exact upsert payload/conflict policy, optimistic visual feedback, timing, burst cleanup, success-only Notes-bar refresh, and zero live side effects. All proof gates passed; the database operation was mocked and no real account or browser action occurred.

## 2026-08-25 — Notes submission owner independent authorization and proof
- Selected the bounded inline `async function submitNote()` owner (`index.html:10649–10684`) as the next protected seam after Notes reactions. Media/music upload, editor construction, visibility UI, expiry cleanup, viewer audio, reactions, reactor lists, realtime, navigation, and account behavior remain excluded.
- Added an exact scoped authorization addendum and pinned rollback artifact for detached proof only. Immutable-origin and Branch2 owner parity passed with SHA-256 `f876963b27ad8661f0609e0dce77d55294e1017d03c88f4c5b9e2bae5de91173`; production extraction remains blocked and protected accounting remains unchanged.
- Added an independent detached VM harness covering empty validation, new-note insert success, active-note update success, insert/update failures, exact payload/visibility/24-hour expiry behavior, success UI ordering, error rollback UI, and zero live side effects. All proof gates passed; insert/update operations were mocked and no real account, storage, upload, media, permission, or browser action occurred.

## 2026-08-25 — Push permission banner owner independent authorization and proof
- Selected the bounded inline `function maybeShowPushPermissionBanner()` owner (`index.html:1003–1059`) as the next safest remaining protected Push seam. Subscription/resubscription owners, service-worker registration, PushManager, VAPID conversion, database persistence, notification delivery, and Settings adapter behavior remain excluded.
- Added an exact scoped authorization addendum and pinned rollback artifact for detached proof only. Immutable-origin and Branch2 owner parity passed with SHA-256 `17186cae241a0283e020fec128598bcbfcbc452ba913f652601f237e1fd0c84b`; production extraction remains blocked and protected accounting remains unchanged.
- Added an independent detached VM harness covering unsupported/granted/denied permission gates, account dismissal, delayed banner creation, logout cleanup, existing-banner suppression, enable grant/denial/error, dismissal persistence, timing, cleanup, and zero live permission/service-worker/PushManager effects. All proof gates passed; permission and subscription operations were synthetic mocks only.

## 2026-08-25 — Silent Push resubscribe owner independent authorization and proof
- Selected the bounded inline `function silentPushResubscribeIfGranted()` owner (`index.html:1068–1076`) as the next protected Push seam after the permission banner. The subscription owner, force-resubscribe flow, service-worker registration, PushManager operations, VAPID conversion, database persistence, notification delivery, and Settings adapter remain excluded.
- Added an exact scoped authorization addendum and pinned rollback artifact for detached proof only. Immutable-origin and Branch2 owner parity passed with SHA-256 `44d86219373553165ac391574670f51de306bbdc5630ce645247dae36d8f932f`; production extraction remains blocked and protected accounting remains unchanged.
- Added an independent detached VM harness covering unsupported capability, default/denied permission, granted logged-in handoff, logout before the deferred callback, exact 5000ms timing, one subscription handoff, and zero direct permission/service-worker/PushManager/database/storage/network/account effects. All proof gates passed; the subscription handoff was a synthetic mock only.

## 2026-08-25 — Push subscription owner independent authorization and proof
- Selected the bounded inline `async function subscribeToPushNotifications()` owner (`index.html:907–943`) as the next protected Push seam after the banner and silent-resubscribe helpers. Permission prompting, banner UI, silent scheduling, force unsubscribe/resubscribe, service-worker registration, notification delivery, and Settings adapter behavior remain excluded.
- Added an exact scoped authorization addendum and pinned rollback artifact for detached proof only. Immutable-origin and Branch2 owner parity passed with SHA-256 `b6f11d4f504f8bc4b3fb7bf47447e8169d093b283f08c6bedaa7bd353adf70b4`; production extraction remains blocked and protected accounting remains unchanged.
- Added an independent detached VM harness covering unsupported browser, missing user, existing subscription refresh, new subscription creation, exact VAPID conversion/options, serialized payload and endpoint conflict policy, device-info truncation, subscribe failure, database failure, get-subscription failure, and zero live permission/service-worker/PushManager effects. All proof gates passed; service-worker, PushManager, and database behavior were synthetic mocks only.

## 2026-08-26 — Push 403/VAPID curve diagnosis from owner-supplied staging evidence
- Reviewed the supplied redacted Supabase Edge Function log from the Branch2 Vercel preview test. It reports `subscriptions found=7`, repeated HTTP 403 failures, `FINAL sent=0/7`, and the explicit body `permission denied: VAPID public key must be on the P-256 curve`; endpoints and private-key values were not committed.
- Independently decoded the only tracked Branch2 client VAPID public key. It is 87 Base64URL characters and 65 decoded bytes with an uncompressed-point prefix, but the point is not on NIST P-256. The pure decoder only converts bytes and cannot correct the key. Diagnosis is configuration-blocked, not a modularization regression.
- Added a detached diagnosis contract, harness, and redacted evidence pin. No VAPID key, private secret, database schema, Edge Function source, account, or production code was changed. Safe remediation requires the project owner to generate/retrieve a valid matching P-256 key pair in the owner-controlled secret manager, update the Edge Function secret and matching client public key, and rerun staging-only tests.

## 2026-08-26 — VAPID diagnosis refinement after generation-method confirmation
- The owner confirmed that the pair was generated with `npx web-push generate-vapid-keys`. A disposable reproduction of that standard command produced an 87-character public key that decoded to a valid P-256 point under Node’s native JWK parser. The exact Branch2 public value still fails the same independent parser, so the refined blocker is a copied/altered/misencoded/stale/different public value in Branch2 or the Edge deployment—not a claim that the standard generator is invalid.
- No private key was retained, exposed, or committed. No client key, Edge secret, database row, account, or production source was changed. The owner-controlled next action is to compare the original generator output with the exact Branch2 client value and Edge configuration, then perform a staging-only subscription/send test after correction.

## 2026-08-26 — Read-only Supabase Edge Function verification
- With the owner’s explicit no-change request, inspected the connected Supabase project read-only. Project ref `ecztpnnydvlzpppmzvzm` contains active function `send-push-notification`, deployed source hash `bc6d3329a401d24008683b62756795091c37bf7b3a924d00ed6cb11b54910050`. The source reads trimmed `VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, and `VAPID_SUBJECT`, passes them to `buildPushPayload`, and posts to each stored subscription endpoint.
- A bounded read-only `function_logs` query directly confirmed seven subscriptions, repeated HTTP 403 responses, the P-256 VAPID error, and `FINAL sent=0/7`. The source and logs confirm the failure is in deployed VAPID configuration/encoding or key material, not in the Branch2 modularization. No secret values, database rows, Edge Function code, deployment, account, or configuration were changed.
## 2026-08-26 — Read-only repeated-rotation wiring audit
- The owner reported that multiple fresh `npx web-push generate-vapid-keys` pairs had already been rotated without changing the 403 behavior. A redacted history check found three distinct public-key fingerprints across tracked `index.html` history, while the current Branch2 client fingerprint `d2ee55d644dd88152d994fd6ac17b17473081c9548d7804b947bb4c7b3221d95` has remained unchanged since commit `6b7a2b7`.
- This does not prove which Supabase secret value was used during prior rotations, but it establishes the key wiring blocker: Supabase-only rotation cannot repair a published client whose embedded public key remains invalid or mismatched. The matching public key must be committed to Branch2 and served by a fresh preview deployment, while the matching private key remains only in Supabase secrets.
- The supplied Vercel preview redirected the sandbox to Vercel login, so the served client asset could not be independently fingerprinted without owner authentication. No login, secret retrieval, external mutation, database/account action, application-source change, or deployment was performed. Detached VAPID diagnosis and Branch2 readiness gates both passed.
## 2026-08-26 — DM/chat static owner-boundary audit
- Selected the remaining DMs/chat system for the next safe preparation step; no production extraction was authorized. Strengthened the existing detached preparation harness to pin `renderDMs()`, `_refreshDmsInPlace()`, `openChat()`, `loadMsgs()`, and `sendMsg()` as inline owners with their current generation guards, parallel base fetches, dependent member lookup, Notes Bar/data-cid DOM markers, screen replacement boundary, navigation-stack handoff, chat/typing subscriptions, and non-destructive refresh boundary.
- The strengthened DM preparation harness, injected realtime harness, and new-DM renderer harness all passed. Protected DM signatures remain inline, extracted DM signatures remain zero, production change remains zero, and live messaging/realtime/account/database/media/browser actions remain zero. Exact before/after parity, safe browser proof, rollback artifact, and explicit product authorization remain outstanding for any production split.
## 2026-08-27 — DMs renderer independent detached proof
- Added a focused independent proof contract, harness, and redacted rollback artifact for the actual inline `renderDMs()` owner. The harness compares the current Branch2 owner with immutable `origin/main`, executes the actual owner in an isolated VM/DOM/database mock, compares a detached candidate across populated, empty, and stale-generation cases, and verifies synthetic dispatcher rollback to the exact original function reference.
- Independent results: `ORIGIN_MAIN_OWNER_PARITY=PASS`, populated render `PASS`, empty state `PASS`, stale-generation abort `PASS`, detached candidate after-parity `PASS`, and synthetic rollback `PASS`. Live network calls, database mutations, account actions, and production extraction remained zero.
- This package does not authorize or perform a production DMs split. Exact production before/after parity, safe authenticated browser proof, production rollback artifact, and explicit feature authorization remain required; DMs protected signatures remain inline and production decision remains blocked.
## 2026-08-27 — VAPID evidence fixture durability after attachment refresh
- The exhaustive regression exposed one harness-only failure because the user reattached `pasted_content.txt` with migration instructions, replacing the earlier redacted Supabase log text. The VAPID diagnosis harness was corrected to read the repository-pinned redacted evidence file `docs/push-vapid-403-diagnosis-evidence.txt` and to accept its normalized `EDGE_STATUS`, `EDGE_ERROR`, `SUBSCRIPTIONS_FOUND`, and `FINAL_SENT` markers.
- This change affects only test-fixture durability and evidence-source normalization. No VAPID secret, application code, Edge Function, database, account, deployment, or production behavior was changed. Focused VAPID and DMs independent proofs pass after the correction; full regression will be rerun after publication.

## 2026-08-28 — Bounded DMs renderer owner externalization and post-split observation
- Under the explicitly authorized bounded DMs scope, moved only the exact inline `renderDMs()` body from `index.html` into the classic external owner `src/features/dms-renderer-owner.js` as `window.renderDMs = async function(){...}`. Added one classic external script reference before the inline application boundary. `openChat()`, `loadMsgs()`, `_loadOlderMessages()`, `_refreshDmsInPlace()`, `_silentBackgroundRefresh()`, typing, send, chat subscriptions, and all other DMs/chat/realtime owners remain inline and protected.
- Published the source split on `Branch2` as commit `211cc1e` (`Extract DMs renderer owner with parity gates`), with `origin/main` preserved at `ef418007c9b9a797488b4825be5f0c807da22369`. The independent detached proof recorded origin-owner parity, populated/empty/stale-generation behavior, detached candidate parity, and synthetic rollback as PASS with zero live effects.
- Authenticated observation-only proof passed before and after the split on the Branch2 preview: Messages/DM list and Notes entry rendered; an existing one-to-one chat opened; header/history/`#mlist`/composer/chat subscription/typing subscription were observed; upward history scroll worked; back restored the DMs list; and the same chat reopened. Message, edit, delete, reaction, call/media, permission, PushManager, service-worker, database, account, and intentional realtime mutations were all zero.
- This milestone does not claim `SPLIT_COMPLETE`: remote rollback-after-split evidence remains pending, and the broader chat/realtime owners remain protected. The synchronized contract/harness/evidence updates in the current worktree must pass the clean full regression gate before the next publication checkpoint.

## 2026-08-28 — DMs renderer remote rollback and recovery proof
- With explicit authorization, created and published temporary rollback checkpoint `94d5fc4`, restoring only the exact `renderDMs()` owner inline and removing its single external script/module. The authenticated preview rendered the DMs list, opened an existing one-to-one chat, rendered history, passed upward history scroll, restored the list on back, and reopened the same chat.
- The rollback observation was strictly non-mutating: message send/edit/delete, reaction, call/media, permission, PushManager, service-worker, database, account, and intentional realtime mutations were all zero. No identifiers, message content, credentials, tokens, endpoints, or secrets were recorded.
- Created and published recovery checkpoint `6faba63`, restoring `src/features/dms-renderer-owner.js` and its classic script tag. The bounded `renderDMs()` owner now satisfies detached parity, authenticated pre/post observation, remote rollback/recovery, focused gates, clean final readiness, and full 315/315 regression. Broader DMs/chat/realtime owners remain protected and VAPID configuration remains a separate blocker.

## 2026-08-28 — Bounded Reels renderer extraction validation checkpoint
- Under the newly recorded explicit production authorization, moved only the exact inline `renderReels()` owner from `index.html` into the classic external owner `src/features/reels-renderer-owner.js` as `window.renderReels = async function(){...}`. The Reels video-windowing helper, navigation stack, swipe handling outside the owner, media/action controls, comments, likes, sharing, uploads, accounts, calls, database, realtime, permissions, and excluded cleanup/policy systems were not moved or behaviorally changed.
- Exact normalized owner parity against immutable `origin/main` passed. Detached populated/empty/error lifecycle, persistent-container, timer-cleanup, forbidden-side-effect, candidate-copy, and synthetic rollback checks passed. Focused Reels seam, protected-owner, classic-script, source-hygiene, and accounting checks passed in the dirty validation tree.
- Current status is `SPLIT_VALIDATION_PENDING`: authenticated post-split browser observation and genuine reversible remote rollback-after-split proof remain required before `renderReels()` can be marked `SPLIT_COMPLETE`. No production deployment, live account/data mutation, secret/configuration change, or Supabase mutation was performed in this checkpoint.
- This entry records validation work only; the source and contract synchronization remains uncommitted until the required browser/rollback gates and clean full regression pass.

## 2026-08-28 — Reels renderer remote rollback and recovery validation
- After the bounded Reels renderer split checkpoint `256e070` was published, temporary rollback checkpoint `3446507` restored the renderer inline and removed only its external owner module/linkage on `Branch2`. The authenticated preview rendered the existing Reels route/card after rollback; the observation remained strictly non-mutating.
- Reels detached parity, lifecycle, swipe/playback/timing/cleanup, navigation-stack isolation, overlay wiring, and synthetic rollback gates passed. Authenticated post-split observation passed for route rendering, safe nested scroll, Home return, and Reels re-entry. Authenticated rollback observation passed for inline restoration, route rendering, existing-card rendering, and route re-entry. Forbidden actions remained zero.
- The inverse recovery was restored exactly from published split checkpoint `256e070` after detecting and correcting an unsafe inverse-script boundary; no downstream inline helpers were retained in the external module. Final recovery publication and clean full regression remain pending at this ledger point.

## 2026-08-28 — Reels renderer bounded split complete
- The authorized bounded `renderReels()` owner is restored as `window.renderReels` in `src/features/reels-renderer-owner.js`, linked by one classic script before the inline application boundary. The surrounding windowing helper, navigation stack, swipe/media policy, action handlers, upload/creation flows, database, realtime, permissions, and account owners remain outside scope.
- Exact immutable-origin parity, detached lifecycle proof, authenticated post-split Reels route/render/scroll/navigation proof, authenticated rollback-after-split proof at rollback checkpoint `3446507`, and recovery publication at `5b6e7cc` all passed with zero forbidden live actions.
- Reels bounded renderer status is `SPLIT_COMPLETE`. The eight unrelated protected systems and excluded Reels navigation/media/action owners remain blocked and require independent authorization and gates.

## 2026-08-28 — Bounded Notes reaction owner extraction validation
- Under the explicit bounded authorization addendum, moved only `reactToNote(noteId, emoji, clickedEl)` from `index.html` to `src/features/notes-reaction-owner.js` as the classic global `window.reactToNote = function reactToNote(...)`, linked exactly once before `note-viewer-owners.js`.
- Immutable-origin owner SHA-256 `3877a5e2a75c4b95ff0e6494a03e7cb9eb2b0f02fee6fe64225c642078986880` passed exact parity. Detached success, missing clicked element, error, payload/conflict policy, optimistic UI, timer cleanup, forbidden-side-effect, protected accounting, and reversible rollback-after-split proofs passed.
- The supplied Branch2 preview was observed safely at the login boundary and was stale relative to this local candidate: it still served the inline owner and had no new module linkage. No reaction was clicked, no real-account action was taken, and live database/realtime/account/media effects remained zero.
- The candidate currently has 230 JS modules, 219 feature modules, 232 script tags, 231 external classic scripts, and 316 harnesses. The final clean full regression is gated on the verified commit and remote alignment; `origin/main` remains immutable.
- Notes reaction status remains `SPLIT_VALIDATION_PENDING` until the committed Branch2 regression and fresh deployment observation are complete. All broader Notes submission/reaction, realtime, media, authentication, DMs, and chat owners remain outside scope.


## 2026-08-28 — Push permission banner independent-proof checkpoint

The bounded `maybeShowPushPermissionBanner()` owner was selected as the safest next protected candidate because its existing authorization allows synthetic detached proof only and explicitly excludes live permission prompts, service-worker access, PushManager operations, database persistence, subscription delivery, authentication, and production extraction. The exact owner boundary remains `index.html:1003–1059`; immutable-origin SHA-256 parity is `17186cae241a0283e020fec128598bcbfcbc452ba913f652601f237e1fd0c84b`. The independent proof passed unsupported/granted/denied gates, account-scoped dismissal, delayed creation, logged-out cleanup, enable-granted/denied/error behavior, dismissal persistence, timing, and cleanup. Live permission requests, live service-worker access, live PushManager access, database writes, network side effects, account mutations, and production changes remained zero; dismissal storage was synthetic-only. Evidence: `docs/push-permission-banner-owner-independent-before-proof-evidence.txt`. Production extraction remains blocked pending a new exact-scope production authorization.


## 2026-08-28 — Push permission banner bounded production split

The project owner explicitly authorized extraction of the bounded `maybeShowPushPermissionBanner()` owner on `Branch2` only. The exact baseline owner from `origin/main` was extracted from `index.html` and moved to `src/features/push-permission-banner-owner.js` as the classic global `window.maybeShowPushPermissionBanner`, with one linkage inserted before the inline application script. The caller and all excluded Push systems remain unchanged.
...
## 2026-08-30 — Silent Push resubscribe bounded production split
The project owner explicitly authorized the bounded production extraction of `silentPushResubscribeIfGranted()` on `Branch2` only, with live permission, service-worker, PushManager, subscription, VAPID, database, storage, network, and account actions excluded. The exact origin owner was extracted from immutable `origin/main` with SHA-256 `44d86219373553165ac391574670f51de306bbdc5630ce645247dae36d8f932f` and moved to `src/features/push-silent-resubscribe-owner.js` as the classic global `window.silentPushResubscribeIfGranted`; one script linkage was inserted before the inline application script, and the inline owner was removed.

The production split harness passed exact origin parity, unsupported capability, default/denied permission, granted logged-in handoff, logged-out callback cleanup, five-second timing, single synthetic subscription handoff, and zero live-effect accounting. The paired contract and production authorization addendum record the one-owner scope and exclusions. Detached reverse-state rollback evidence passed in a disposable directory, and the full 318-harness regression passed with 316 behavioral/contract harnesses plus the two expected clean-publication gates. Fresh deployment browser observation passed on the Branch2 Vercel preview: the module source was present with the classic `window` assignment, the login boundary loaded `window.silentPushResubscribeIfGranted` as a function, the owner was not invoked, and all live-effect counters remained zero.

## 2026-09-01 — Notes submission bounded production split
The project owner explicitly authorized the bounded production extraction of `submitNote()` on `Branch2` only, limited to the exact owner boundary and exclusions in the Notes submission production authorization addendum. The exact immutable-origin owner was extracted from `index.html:10143–10179` and moved to `src/features/notes-submission-owner.js` as the classic global `window.submitNote`; one script linkage was inserted and the inline owner was removed.

The production split harness passed exact origin parity with SHA-256 `f876963b27ad8661f0609e0dce77d55294e1017d03c88f4c5b9e2bae5de91173`, empty validation, insert/update success, insert/update error, exact payload, visibility, expiry, UI ordering, rollback behavior, and zero live effects. Disposable rollback restored the pre-split Branch2 baseline byte-for-byte. Safe browser observation and the full post-push regression remain final publication gates; live Note creation/update, database, storage, upload, media, permission, service-worker, Push, network, account, and real-browser actions remain excluded.

## 2026-09-03 — Push subscription bounded production split
The project owner explicitly authorized the bounded production extraction of `subscribeToPushNotifications()` on `Branch2` only, with live Push, service-worker, PushManager, permission, database, storage, network, and account actions excluded. The exact immutable-origin owner was extracted from `index.html:912–948` with approved SHA-256 `b6f11d4f504f8bc4b3fb7bf47447e8169d093b283f08c6bedaa7bd353adf70b4` and moved to `src/features/push-subscription-owner.js` as the classic global `window.subscribeToPushNotifications`. One script linkage was inserted at `index.html:415`, positioned after `url-base64-to-uint8-array.js` and before `push-silent-resubscribe-owner.js` to preserve dependency and script order.

The production split harness passed exact origin parity, unsupported-browser gate, missing-user gate, existing-subscription refresh, new-subscription creation, VAPID and subscribe options, DB payload and endpoint conflict policy, subscribe failure, database failure, get-subscription failure, and device-info truncation, all with zero live effects (mocked DB only). Disposable rollback restored the pre-split Branch2 baseline byte-for-byte at SHA-256 `026160e0565c8f894dfac0908797fe9b285e0c474ede7b08b374826daa14a5bb`. The harness-path portability fix (commit `0f225d3`) was published first to make the local test suite env-aware; the extraction itself sits at the next Branch2 commit. Static browser observation passed (script tags balanced, single linkage, zero inline owner, module parses, dependency order preserved); deployed browser observation remains pending a future agent with preview access. The full 319-harness regression passed after extraction. Live permission requests, service-worker access, PushManager access, database writes, storage writes, network side effects, and account mutations remained zero.

## 2026-09-03 — Push force-resubscribe bounded production split
The project owner explicitly authorized the bounded production extraction of `forceResubscribePush()` on `Branch2` only, with live Push, service-worker, PushManager, permission, database, storage, network, and account actions excluded. The exact immutable-origin owner was extracted from `index.html:924–957` with approved SHA-256 `6f57c4e0fc347b63d158739a184e0f3f8323ed7c6b57528d0eb833aaaaa4d63d` and moved to `src/features/push-force-resubscribe-owner.js` as the classic global `window.forceResubscribePush`. One script linkage was inserted after `push-subscription-owner.js` and before `push-silent-resubscribe-owner.js` to preserve dependency order (forceResubscribePush calls the already-external subscribeToPushNotifications).

The production split harness passed exact origin parity and all 9 synthetic scenarios: unsupported-browser gate, missing-user gate, existing-subscription cycle (unsubscribe + DB delete + fresh subscribe), no-existing-subscription path, unsubscribe failure, DB delete failure, get-subscription failure, fresh-subscribe success, and fresh-subscribe failure — all with zero live effects (mocked DB only, no real Push/SW/PushManager access). Disposable rollback restored the pre-split Branch2 baseline byte-for-byte at SHA-256 `e919398e5de0fe52b884a2fbc405e15d0add62e72bb56dd5d0ea0534e49cd1a9`. Local HTTP server browser observation passed (237 script tags balanced, all 235 local scripts + 18 CSS resolve HTTP 200, module parses, dependency order preserved, window.forceResubscribePush exposed as function). The full 322-harness regression passed after extraction. Live permission requests, service-worker access, PushManager access, database writes, storage writes, network side effects, and account mutations remained zero.

## 2026-09-04 — Final-stretch autonomous extraction batch ledger restoration

This entry restores the migration-map ledger coverage for the autonomous final-stretch extraction batch (234 commits from the Push force-resubscribe publication through the forward-message owner, plus the nova-ultra-patches region split). During that batch the extraction guards, parity hashes, count-sync harness updates, and full regressions were executed per owner, but this chronological map was not appended per checkpoint. The batch is summarized here so the ledger remains the authoritative continuation record.

- Batch range: commits after the push-force-resubscribe documentation checkpoint up to the nova-ultra-patches extraction. All work was published on `Branch2` only; `origin/main` remained immutable at `ef418007c9b9a797488b4825be5f0c807da22369` throughout, verified before and after every extraction.
- Protected-owner families completed in the batch under the standing autonomous production authorization (one owner at a time, guarded extractor, exact-parity and rollback guards, count-sync, full regression, app-load test): Calls/WebRTC (createPeerConnection, endCall, setupWebRTCCaller, setupWebRTCCallee, initiateCall, handleIncomingCall, acceptIncomingCall, rejectIncomingCall, and the group-call/listener owners), voice recording (toggleRecording), chat/DMs runtime (openChat, sendMsg, showMsgMenu, showMsgMenuFromEl, forwardMessage, completeForwardMessage, loadMsgs, deleteMsgForMe, unsendMsg, clearChat, and the typing/scroll/pill owners), media upload pipeline (upload, submitCreate, deleteMediaProduction, uploadToCloudinary, attachLocalVideoStream), Story viewer system (openSV, renderSV, closeSV, nextSV, prevSV, nextUserSV, prevUserSV, stopSVPlayback, voteStoryPoll, refreshPollResults, loadStoryPollState, showStoryViewers, downloadStory, publishStoryEditor, showCreateStory, reactToStory, sendStoryReply, showStoryActions, shareStoryAsPost), tab caching (_saveTabToCache, _tryRestoreFromCache, invalidateTabCache), auth/bootstrap helpers (showApp, syncCurrentAccountToSavedList, startBanRecheck, signOutBanned), and the complete admin/moderation surface.
- The batch closed with the forwardMessage owner extraction, which the guarded extractor verified as the final remaining inline function declaration: after that commit, `index.html` contained zero top-level function declarations.
- Validation per extraction: guarded extractor pre-write guards (worktree clean, Branch2 alignment, immutable origin/main, single owner boundary, balanced braces), post-transform verification (script-tag counts, linkage order, module syntax, remaining inline syntax), byte-for-byte rollback proof, count-sync of the standard count harnesses, full 322-harness regression, and the 10-check app-load test with all local scripts and CSS resolving HTTP 200.

## 2026-09-04 — Nova Ultra v4/v5 patch-region bounded production split

The remaining inline application code was audited after the final function extraction. The audit found: (1) shared global state declarations (tab cache, story viewer, reels, call, FAB, crop, story editor, notes/audio state, VAPID public key, media config, filters, notification preference map) that must stay inline as the global lexical environment consumed by earlier and later classic scripts; (2) the protected bootstrap wiring (service-worker registration, the main `load` authentication/session/deep-link handler, and `onAuthStateChange`); (3) three global event listeners (two FAB outside-tap closers and the notes audio `visibilitychange` pauser) covered by the event-listener boundary contract; and (4) the Nova Ultra v4/v5 top-level patch region — guarded window-owner overrides for `loadMoodFeed`, `showNovaUniverseHub`, `generateAICaption`, `getLocalAIResponse`, `handleNovaCommand`, and `initNovaFeatures`, plus the inert `toggleLike` particle guard.

Only (4) was production-extractable. The exact region from the "PARTICLE EFFECT ON LIKE" banner through the closing brace of the generateAICaption reels-guard patch (region SHA-256 `89ef28fd0f429b1b205230e6c7fb5118edafe29c0e1a12a575c8eaf1e2056476`) was moved verbatim (modulo trailing-whitespace hygiene trimming) into `src/features/nova-ultra-patches.js` as a classic script. One linkage was inserted immediately after `destroy-reels-persistent-container.js`, preserving load order: every patch target (`nova-ai.js`, `local-ai-response.js`, `ai-generators.js`, `smart-feed.js`, `nova-universe.js`, `ai-context.js`) loads earlier, and the module still runs before the inline application script.

The guarded extractor verified: region window assignments = 9 (all names already in the window-assignment allowlist, combined surface count unchanged at 449), region top-level lexical declarations = 8 with zero cross-module collisions, zero addEventListener/setInterval/localStorage in the region, braces balanced (70/70), byte-for-byte rollback reversibility, and syntax validity of both the new module and the remaining inline script. The detached proof additionally verified load order, patch application against stub targets in a VM, and HTTP 200 serving of the module from a local server, with zero live effects. The unique inline onclick handler inventory fell from 18 to 16 (the `goBack` and `showSmartFeed` template-string handlers moved with the region and still resolve in modules). The pre-existing inert guards (`toggleLike` and `initNovaFeatures` wrappers whose targets load after the module) are preserved verbatim as pre-split behavior.

After this split the inline application script contains only the by-design state declarations, bootstrap wiring, and boundary event listeners: zero functions, zero patch overrides. The full regression passed at 320/322 with only the two publication gates (clean-worktree and remote-alignment assertions) pending the commit-and-push of this checkpoint; the app-load test passed 10/10 with 463 local script references, 465 balanced script tags, 452 feature files, and all 463 scripts syntax-valid.

## 2026-09-05 — Publication gate closure and fresh-session structural audit

The nova-ultra-patches checkpoint `06efac5` was published on `Branch2`, and a fresh-session continuation agent re-ran the complete verification from the clean published checkout: worktree clean, HEAD = `origin/Branch2` = `06efac5`, immutable `origin/main` = `ef418007c9b9a797488b4825be5f0c807da22369`, full regression **322/322 PASS** (both publication gates closed), and app-load test **10/10 PASS**. No application code was changed for this verification.

The same session performed the structural audit the project owner requested:

- **Remaining split work in `index.html`: none.** The file contains zero function declarations. The single remaining inline application script holds only the by-design boundary surface — shared global state declarations, the protected bootstrap wiring (service-worker registration, main `load` handler, `onAuthStateChange`), and the three boundary event listeners — plus 18 unique inline HTML handler attributes that are part of the runtime contract and resolve against the extracted global owners at click time.
- **Duplicate-owner audit across all 463 source modules:** 35 window-assigned names are written in more than one file. 33 of these are shared global state flags (`_pendingIceCandidates`, `_chatMembers`, `_forwardMessagePending`, and similar) written by multiple feature modules, which is the normal classic global architecture, not duplication. The remaining 2 are documented intentional patch chains: `showApp` (defined in `show-app.js`, wrapped by the `nova-init.js` seam) and `initNovaFeatures` (patched by `ai-moderation.js` and then by `nova-ultra-patches.js`). Zero accidental duplicate function owners were found.
- **Per-feature consolidation review (deferred to the project owner, not executed):** Merging the ~48 Calls/WebRTC one-owner modules into a single calls module (or analogous merges for other families) was evaluated and is not recommended under the standing safety rules. The merge would reverse the completed one-owner-per-file modularization, invalidate the pinned inventory counts (463 source files, 452 feature files, 465 script tags) and module-read assertions across the 322 harnesses, and risk load-order regressions because every script linkage was inserted at a dependency-exact position between unrelated modules. A directory-level reorganization (for example `src/features/calls/`) would preserve file contents but still rewrite approximately 463 script paths and many module-read harness references. Either variant requires its own explicit production authorization, a full path/count sync plan, and rollback evidence before any execution.

The audit evidence is recorded by the read-only auditor `/home/z/my-project/scripts/analyze_structure.js`, the 2026-09-05 continuation-log entry in `HANDOFF.md`, and this ledger entry. Zero live application, database, storage, upload, permission, service-worker, Push, network, account, or authentication actions were performed.

## 2026-09-05 — Codebase Health & Integrity Audit (read-only)

The project owner commissioned a complete read-only health and integrity audit of the Branch2 repository at checkpoint `6a4d591` (verified clean, aligned with `origin/Branch2`, immutable `origin/main` = `ef418007c9b9a797488b4825be5f0c807da22369`). The audit changed no production code, deleted nothing, renamed nothing, and reordered no scripts; its deliverable is the report `docs/CODEBASE_HEALTH_AUDIT.md`, published with this entry.

The audit method combined custom read-only static analyzers (content hashing for duplicate files, window-owner and declaration census, event-listener and Supabase-realtime inventory, broken-reference resolution across HTML/CSS/service-worker/manifest/docs, dead-code and CSS usage analysis, and security pattern scanning) with a detached Node `vm` simulation that loaded all 464 local classic scripts in exact `index.html` order under absorbing DOM/Supabase stubs, plus the published 322/322 harness regression and the 10/10 app-load test. All checks ran with zero live effects.

Headline results: zero critical issues; zero broken references; zero syntax errors; zero load-time errors across the full 464-script VM simulation (which also proves zero cross-script lexical collisions); zero accidental duplicate function owners (the two multi-file function owners are the documented `showApp` and `initNovaFeatures` patch chains, and 33 shared state flags are by-design architecture); zero duplicate realtime channels (10 unique channels, each with exactly one creation site and owned cleanup); zero never-unsubscribed subscriptions; one correct auth listener; zero unreferenced files; zero committed secrets (the Supabase key is the public `anon`-role key; `window.ZAI_API_KEY` is never committed and injected at runtime).

Three high-risk findings were verified: (1) the smart-reply quick-send buttons render but do nothing because `smart-replies.js:34` reads `cinp` while the chat input is `minp` (`open-chat.js:143`); (2) the video-length picker UI never renders because `video-length-options.js:3-4` requires `vlenpick`/`vlen-opts` containers that no template creates, although `showVideoLengthOptions()` is invoked from `prev-media.js:12`; (3) a systemic XSS surface of 398 `innerHTML` assignments (72 with direct interpolation, roughly ten interpolating user-controlled text such as typing usernames, chat text, pinned text, story text, and search names) with no `escapeHtml` helper defined anywhere in the repository. Medium findings include the dead notification-badge chain (`notif-dot` never created, `home-notif-dot` never updated), the dead client-side comment-moderation pre-check (`ai-moderation.js:23` reads `cinp`; the comment input is `ci-<postId>`), the dead optimistic following-count update (`following-count` never created; the real stat is `followers-count`), one `eval()` at `profile-view.js:548`, an unversioned service-worker cache (`novasocial-v1`, cache-first) with post-deploy staleness risk, repo-hygiene items (a 1-byte junk root file `chore: add feature architecture` from commit `90a57d8` and 14 empty feature scaffolding directories), and the dead legacy `addStoryTextMode` function. The report's nineteen sections record every finding with severity, file, line, impact, recommended fix, and automation-safety, closing with a recommended fix order and totals.

Publication of the report is accompanied by the deliberate inventory count-sync required by the published protocol: `branch2-final-readiness-contract-harness.js` documentation pins moved from 338 to 339 Markdown documents and the non-contract documentation allowlist gains `CODEBASE_HEALTH_AUDIT.md`. No harness logic or assertion strength was weakened. Zero live application, database, storage, upload, permission, service-worker, Push, network, account, or authentication actions were performed during the audit or this publication.

## 2026-09-05 — Audit fix 1 (H1): smart-reply quick-send wiring restored (cinp → minp)

The project owner authorized implementing the fixes recommended by `docs/CODEBASE_HEALTH_AUDIT.md`, one bounded change per commit with the full 322-harness regression after each. This checkpoint applies fix H1: the smart-reply quick-send buttons rendered but did nothing because `quickSendReply` (`src/features/smart-replies.js:34`) looked up the stale chat-input id `cinp`, while the real chat textarea created by `open-chat.js:143` carries `id="minp"`. The change is exactly one line: `document.getElementById('cinp')` → `document.getElementById('minp')` inside `quickSendReply`, restoring the insert-and-send behavior of the rendered reply chips.

No other behavior, global, load order, or file inventory changed. The `getSmartReplies` pure classifier and its contract harness are untouched (the harness pins only the classifier, not the sender). The deliberate contract syncs in this commit: `docs/branch2-only-safety-contract-harness.js` allowed-checkpoint list gains `src/features/smart-replies.js` (the same mechanism every extraction checkpoint used to admit its module), and its cosmetic `LATEST_CHECKPOINT` label moves from the stale `NOVAULTRAPATCHES_EXTRACTION` to `FIX1_SMART_REPLY_WIRING` (the audit's L3 drift). Zero live application, database, storage, upload, permission, service-worker, Push, network, account, or authentication actions were performed.

## 2026-09-05 — Audit fix 2 (M2): comment moderation pre-check restored (cinp → ci-<postId>)

Fix 2 of the owner-authorized audit-fix series applies the audit's M2 finding: the client-side comment-moderation pre-check installed by the `sendCmt` wrapper in `src/features/ai-moderation.js:23` never ran because it looked up the stale id `cinp`, while the real per-post comment input created by `src/features/comments.js:63` carries `id="ci-<postId>"`. The change is one line inside the wrapper — `document.getElementById('cinp')` → `document.getElementById('ci-'+pid)` — using the `pid` argument the wrapper already receives, exactly matching the original `sendCmt` owner's own lookup at `comments.js:87`. Flagged comments are again blocked client-side with the community-guidelines toast before the original sender delegates.

The deliberate contract sync in this commit: `docs/ai-moderation-contract-harness.js` marker list updates its pinned input-lookup string from `document.getElementById('cinp')` to `document.getElementById('ci-'+pid)`. This is a content sync of the pinned marker to the corrected wiring, not a weakening: the marker assertion (source must contain the lookup), the moderation-flag/delegation/feedback assertions, and all forbidden-scope assertions keep identical strength, and the contract's intent — the wrapper pre-screens the comment input before delegating — is now actually true at runtime. `docs/branch2-only-safety-contract-harness.js` gains `src/features/ai-moderation.js` in its allowed-checkpoint list and its `LATEST_CHECKPOINT` label moves to `FIX2_COMMENT_MODERATION_PRECHECK`. Zero live application, database, storage, upload, permission, service-worker, Push, network, account, or authentication actions were performed.

## 2026-09-05 — Audit fix 3 (M3): optimistic Following-count stat wiring restored

Fix 3 of the owner-authorized audit-fix series applies the audit's M3 finding: the optimistic following-count update never fired because no template ever created the `following-count` element its consumers look up (`update-my-following-count.js:3` and the `following-count` branch of the parity-pinned `refresh-profile-counts-owner.js:12`). The fix mirrors the already-working `followers-count` wiring exactly: the Following stat in the other-user profile stats row (`src/features/profile-view.js:395`) and in the own-profile stats row (`src/features/profile.js:107`) now carry `id="following-count" data-raw="<raw count>"`, with `data-raw` holding the raw numeric value (`prof.following_count`/`PROF.following_count`) exactly as the Followers stat holds its raw value.

With this wiring, `toggleFollowProfile`'s optimistic `updateMyFollowingCount(±1)` mutates the stat in place via `dataset.raw` and `fmt()` (clamped at zero), the post-write `refreshProfileCounts(userId)` re-syncs it from the database, and the error path rolls it back — no full page refresh needed. The two parity-pinned consumer modules are untouched (the refresh-profile-counts owner remains byte-identical to its origin/main owner, enforced by its production-split harness). One documented design nuance for the project owner: `refreshProfileCounts` writes the viewer's own `ME.following_count` into the `following-count` element wherever it exists, per the pinned original owner behavior; on the own-profile view this is exactly correct, and on the other-user view the element now receives that same authoritative database value after a follow toggle instead of staying stale. Any further refinement of which user's count the other-user view should display is a product decision deferred to the owner. `docs/branch2-only-safety-contract-harness.js` gains `src/features/profile-view.js` and `src/features/profile.js` in its allowed-checkpoint list and its `LATEST_CHECKPOINT` label moves to `FIX3_FOLLOWING_COUNT_STAT_WIRING`. Zero live application, database, storage, upload, permission, service-worker, Push, network, account, or authentication actions were performed.

## 2026-09-05 — Audit fix 4 (M1): notification unread badge wiring restored (option a)

Fix 4 of the owner-authorized audit-fix series applies the audit's M1 finding using the project owner's selected option (a) — the simplest variant that matches the existing bare-dot topbar design. The unread-badge chain was dead end-to-end: the count setter (`checkUnreadNotifs`, `notifications.js:11`), the post-read hider (`renderNotifs`, `notifications.js:204`), and the realtime INSERT shower (`setupNotifsRealtime`, `notifications.js:327`) all targeted a `notif-dot` element that no template ever creates, while the dot the home topbar actually creates (`home-notif-dot`, `home.js:103`, an 8px bare dot) had no updater at all.

The fix rewires every consumer to the real element and restores the update logic: (1) all four `notif-dot` lookups (`notifications.js:11,204,327` and the dynamic-island wrapper reads in `nova-universe.js:148-151`) now look up `home-notif-dot`; (2) because the real element is a bare 8px dot with no room for text, the count logic now toggles the dot (`display:block` when unread > 0, `display:none` otherwise) and records the raw count in `dataset.count` instead of writing `textContent`; (3) the home renderer now calls `checkUnreadNotifs()` after every topbar render (`home.js`, after `setupHomePullToRefresh()`), so the badge state is restored from the database on every Home visit — previously the template re-created the dot hidden on each render with nothing to re-show it. `renderNotifs` continues to hide the dot after marking notifications read, and the realtime INSERT handler continues to show it on new arrivals.

One pre-existing defect was discovered during verification and deliberately left untouched for the project owner's decision (it is inherited verbatim from `origin/main` and is not a regression of the split): the dynamic-island wrapper in `nova-universe.js:145-156` declares `_origCheckUnread_v2` but guards on the never-defined `_origCheckUnread`, so that wrapper never installs and the island feature has never run — the identical dead-guard pattern also exists in `ai-moderation.js:21` (`_origSendCmt`) and `ai-moderation.js:44` (`_origInitNova`) in the original monolith. The badge fix itself does not depend on that wrapper (the base `checkUnreadNotifs`/`renderNotifs`/`setupNotifsRealtime` owners are live), and the wrapper's element reads were still renamed for consistency. `docs/branch2-only-safety-contract-harness.js` gains `src/features/notifications.js`, `src/features/nova-universe.js`, and `src/features/home.js` in its allowed-checkpoint list and its `LATEST_CHECKPOINT` label moves to `FIX4_NOTIFICATION_BADGE_WIRING`. Zero live application, database, storage, upload, permission, service-worker, Push, network, account, or authentication actions were performed.

## 2026-09-05 — Audit fix 5 (H2): video length picker container restored in the create modal

Fix 5 of the owner-authorized audit-fix series applies the audit's H2 finding: the video length picker never rendered because `showVideoLengthOptions(duration)` (`src/features/video-length-options.js:3-4`) requires `vlenpick` and `vlen-opts` containers that no template ever created, even though the renderer is correctly invoked on video metadata load (`prev-media.js:12`). The fix takes the owner's first-listed option — recreate the container in the media-preview template — with the least-disruptive placement: the create modal template (`src/features/create.js`, the `showCreate` body for post/reel) now includes `<div id="vlenpick" ...><div id="vlen-opts" ...></div></div>` as a hidden pill row immediately after the `post-edit-tools` row and before the `filter-tray`, matching the modal's existing horizontal-pill UI pattern.

The placement is a sibling of the preview box rather than a child of `#mprev`, because `prevMedia` replaces `#mprev`'s inner HTML with the video/img element on every selection — a child container would be destroyed on each media load, while the sibling survives and matches the existing image-path reference that hides `vlenpick` (`prev-media.js:18`). With the container present, the full chain is restored end-to-end: picking a video probes metadata, `showVideoLengthOptions` renders the eligible presets (15/30/60/90/180 filtered by duration, plus the full-length pill within `MAX_VIDEO_LEN`, with the 3-minute toast and 180s auto-select for longer videos), and `selectVideoLen` toggles the pill selection into `window._videoTrimTo`. Picking an image hides the row, and re-opening the modal re-creates it hidden.

The renderer module itself is untouched — its contract (`docs/video-length-options-contract.md`, "the existing module remains unchanged"; missing-container tolerance) and detached harness continue to hold verbatim, and the change only supplies the real-world container the renderer always expected. `docs/post-creation-flow-contract-harness.js` markers for the create template are all preserved (the new row adds no functions and removes no pinned markers). `docs/branch2-only-safety-contract-harness.js` gains `src/features/create.js` in its allowed-checkpoint list and its `LATEST_CHECKPOINT` label moves to `FIX5_VIDEO_LENGTH_PICKER_CONTAINER`. Zero live application, database, storage, upload, permission, service-worker, Push, network, account, or authentication actions were performed.

## 2026-09-05 — Audit fix 6 step 1 (H3, adjusted): escape-helper record correction — the shared helper already exists

Fix 6 step 1 of the owner-authorized audit-fix series was prescribed as "add `function escapeHtml` to `src/core/utils.js` and expose it as `window.escapeHtml`." Verification before writing it found the audit's premise factually incorrect: the repository already owns a shared HTML-escaping helper with exactly the prescribed semantics — `esc()` at `src/core/utils.js:4-12`, inherited verbatim from `origin/main` (monolith line 1432), pinned by `docs/escape-helper-contract-harness.js` (exactly one shared definition, nullish handling, stringification, and the `& < > " '` entity set verified in a detached VM), and already in active use by the admin-side renderers (`load-admin-content.js`, `load-appeals-list.js`, `show-admin-user-detail.js`, `load-reports-list.js`). The audit's security scan had searched for the name `escapeHtml` only and missed it.

Adding a new `function escapeHtml` would therefore create a duplicate owner of an existing single-owner helper — the exact defect class this repository's whole modularization discipline exists to prevent — so this checkpoint takes the adjusted step instead: a documentation-only correction. `docs/CODEBASE_HEALTH_AUDIT.md` gains a dated post-publication corrections section stating the corrected H3 premise, and the staged wrapping series that follows (fix 6 step 2, one file per commit) will wrap the prescribed user-controlled interpolations with the existing `esc(...)` call, which is byte-for-byte the same protective function the plan intended. No production code changes in this commit; `docs/branch2-only-safety-contract-harness.js` only updates its `LATEST_CHECKPOINT` label to `FIX6_STEP1_ESCAPE_HELPER_RECORD_CORRECTION`. Zero live application, database, storage, upload, permission, service-worker, Push, network, account, or authentication actions were performed.

## 2026-09-05 — Audit fix 6 step 2, wrap 1 (H3): typing-indicator username escaped

First staged wrap of the owner-ordered XSS rollout: the chat typing indicator (`src/features/start-typing-watcher.js:31`) now renders the realtime peer username through the shared `esc()` helper — `payload.new.username` is the typing user's profile username (cross-user content over the realtime profiles channel), previously interpolated raw into `innerHTML`. Text-content context, so single `esc()` escaping is the correct form. Load order is safe: `src/core/utils.js` (the `esc` owner) loads with the core group before every feature module. Zero live application, database, storage, upload, permission, service-worker, Push, network, account, or authentication actions were performed.

## 2026-09-05 — Audit fix 6 step 2, wrap 2 (H3): optimistic chat text escaped

Second staged wrap: the optimistic chat bubble (`src/features/send-msg.js:33`) now renders the outgoing message text through the shared `esc()` helper before insertion into `innerHTML`. The value is the sender's own composed text, so this primarily closes the self-XSS reflection vector and hardens the bubble against any future reuse of the path; the raw `txt` continues to flow unchanged to the database insert, mention detection, and rollback logic. Text-content context, single `esc()` form. The wrap-1 checkpoint's handoff continuation entry is folded into this checkpoint's handoff update (same series, adjacent one-line wraps). Zero live application, database, storage, upload, permission, service-worker, Push, network, account, or authentication actions were performed.

## 2026-09-05 — Audit fix 6 step 2, wrap 3 (H3): pinned-message bar text escaped

Third staged wrap: the chat pinned-message bar (`src/features/pin-msg.js:9`) now renders the pinned text through the shared `esc()` helper. The pinned message can originate from the other chat participant, so this closes a genuine cross-user XSS vector in the pin bar; the raw text still flows unchanged to the `conversations.pinned_message_text` update. Text-content context, single `esc()` form. This checkpoint also restores the per-series handoff continuation entry (wraps 1-3 summarized) after the two prior one-line wrap commits carried only their migration-map entries. Zero live application, database, storage, upload, permission, service-worker, Push, network, account, or authentication actions were performed.

## 2026-09-05 — Audit fix 6 step 2, wrap 4 (H3): feed caption attribution and expansion escaped

Fourth staged wrap — the highest-traffic surface in the series, the feed caption line. `formatCaption` in `src/features/posts.js` now escapes every user-controlled interpolation: the author attribution username and the caption body in the short path (single `esc()` for plain text contexts, also improving the `undefined`-username edge case to an empty string), the truncated `shortText` in the long path, and — for the "more" expander's inline onclick, which rebuilds the caption HTML through a JS string inside an HTML attribute — a deliberate double escape `esc(esc(username))` / `esc(esc(caption))`. The double form is required by that context's three decoding layers: the browser decodes entities in the attribute value, the single-quoted JS string must survive without raw quotes, and the final `innerHTML` assignment must render the entities as literal text; a single escape would be decoded back to raw markup at the first layer, and the previous hand-rolled quote-only `safeCaption` replacement left `<`/`>` fully injectable on expansion. The feed rendering caller (`posts.js:163`) is unchanged. Zero live application, database, storage, upload, permission, service-worker, Push, network, account, or authentication actions were performed.

## 2026-09-05 — Audit fix 6 step 2, wrap 5 (H3): story editor user-text elements escaped

Fifth staged wrap: the story editor element renderer (`src/features/story-editor-owners.js`, `renderStoryElements`) now escapes every user-authored text interpolation through the shared `esc()` helper — the text-element body (line 15), text-sticker body (line 18), emoji-sticker body (line 20), poll option labels (line 25), poll question (line 28), and the mention/location/hashtag/link element body (line 31). All are plain text-content contexts, so the single `esc()` form is correct. Editor-selected styling values (font families, weights, colors, sizes) remain interpolated as before — they come from fixed picker options, not free text, and are outside the audit's prescribed scope. The double-tap text edit flow is unaffected: it loads the raw `el.text` into the input value, and only display rendering is escaped. The safety-contract module marker and the story parity harness's text-preservation assertion (plain 'Hello' text) remain satisfied. Zero live application, database, storage, upload, permission, service-worker, Push, network, account, or authentication actions were performed.

## 2026-09-05 — Audit fix 6 step 2, wrap 5 amendment: story parity harness dependency sync

The wrap 5 checkpoint (6524c78) escaped the story editor's user-text interpolations, and the detached `docs/story-editor-browser-parity-harness.js` — the only harness that executes `renderStoryElements` in a VM — correctly caught that its synthetic context did not provide the now-required global `esc()` dependency that production supplies through `src/core/utils.js` loading before every feature module. (The pre-commit verification pipe masked the failure, so the wrap checkpoint was published before the harness ran red; this amendment publishes the fix immediately after.) The harness now loads the REAL `esc` source extracted from `src/core/utils.js` into its VM context before loading the module — deliberately not a mock — and asserts the injected dependency is the genuine helper (`esc('<b>"&')` equals the full entity form). All other story harnesses reference the renderer only through string markers and were unaffected. No assertion was weakened: the existing renderer assertions are unchanged, and the dependency injection plus the real-helper assertion are additions. Zero live application, database, storage, upload, permission, service-worker, Push, network, account, or authentication actions were performed.
...
## 2026-09-05 — Audit fix 6 step 2, wrap 9 (H3): explore no-results query echo escaped

Ninth and final staged wrap of the owner-ordered series: the explore search no-results echo (`src/features/explore.js:140`) now renders the echoed search query through the shared `esc()` helper, closing the reflected-XSS vector in the user's own query text. The explore contract harness's pinned marker text (`ke liye koi result nahi mila`) and all query/limit/category pins are preserved. With this checkpoint the prescribed highest-touch wrapping list is complete: typing indicator, optimistic chat text, pinned text, feed caption attribution and expansion, story editor text elements, all three search-result name surfaces, and the explore query echo are now escaped through the single shared `esc()` owner, each published as its own one-file commit with a full 322-harness regression after every commit. The audit's remaining lower-priority interpolation sites (roughly 60 of the 72 interpolated assignments, including message-list rendering, notification bodies, and comment rendering) remain open for a follow-up series with the owner's authorization. Zero live application, database, storage, upload, permission, service-worker, Push, network, account, or authentication actions were performed.

## 2026-09-05 — Audit fix 7 (M4): profile action-sheet eval replaced with a dispatch table

Fix 7 of the owner-authorized audit-fix series removes the repository's only `eval()` (audit M4, `src/features/profile-view.js`). `showUserProfileOptions` previously built per-action command strings interpolating `userId` into code such as `blockUser('<uuid>',null);document.getElementById('action-sheet-bg').remove()` and executed them with `btn.onclick = () => eval(a.action)`. The refactor inspects the actual actions array and maps each entry to a `type` key consumed by a local `profileActionHandlers` dispatch table whose entries call the real global owners with direct arguments: `shareUserProfile(id)`, `showCloseFriends()`, `muteUser(id, null)`, `blockUser(id, null)`, `unblockUser(id, null)`, `toast('User restricted')`, and `reportUser(id)`.

Behavior is preserved exactly: the same seven action rows render with the same icons, colors, labels, and block-state-conditional labeling; the Share and Close Friends actions keep the sheet open while every other action calls the main function first and then removes the sheet, matching the original statement order; the block/unblock branch selects its handler the same way the label does. The sheet removal moved into a `closeProfileActionSheet` helper with a null guard (the original inline removal would have thrown on a missing element). The `userId` no longer passes through a string-interpolation layer at all, and the optional-chaining dispatch (`?.`) keeps unknown types inert. With this checkpoint the repository contains zero `eval()` calls, removing the code-execution primitive the audit flagged and unblocking future CSP hardening. `go-back.js`'s reference to the `action-sheet-bg` id is unaffected. Zero live application, database, storage, upload, permission, service-worker, Push, network, account, or authentication actions were performed.

## 2026-09-05 — Audit fix 9 (M6/M7/L1/L2/L4): hygiene batch

Final fix of the owner-authorized audit-fix series — the combined hygiene commit the owner prescribed as one batch: (1) removed the 1-byte junk root file `chore: add feature architecture` (a shell accident from commit 90a57d8, flagged by the duplicate-file check); (2) removed the dead legacy `addStoryTextMode()` function from `src/features/story-text-helpers.js` — it had zero call sites and unguarded references to the nonexistent `story-prev`/`story-text-tools`/`story-submit-btn` ids, so it would have crashed if ever revived; the live helpers in the file (including `prevStoryMedia` with its pinned object-URL creation) are untouched; (3) removed the three guarded no-op `react-box` lookup-and-remove statements from `pin-msg.js`, `unsend-msg.js`, and `message-clipboard-helpers.js` — the `react-box` element is never created anywhere, so each lookup always failed its guard and did nothing; (4) fixed the harness filename drift in `docs/contract-artifact-pairing-contract.md` (the referenced harness is `contract-artifact-pairing-contract-harness.js`, with the standard `-contract-harness` infix); (5) corrected the three stale assertion message strings in `docs/branch2-final-readiness-contract-harness.js` (the pinned values 322/318/317 were already correct; only the message text lagged at 321/317/316).

Deliberate contract syncs in this commit: `docs/message-clipboard-helpers-contract-harness.js` drops its now-removed `react-box` marker, gains an assertion that the stale lookup STAYS removed, and its scope label drops the REACTION_CLEANUP segment — the marker list and assertion strength are otherwise unchanged. Per the owner's instruction the 14 empty `src/features/<domain>/.gitkeep` scaffolding directories remain untouched (reorganization decision still open), the DevTools-callable `showNavDebugLog`/`clearNavDebugLog` diagnostics in `src/core/navigation.js` remain in place, and the service-worker cache versioning (fix 8) is deliberately skipped as deploy-gated. Zero live application, database, storage, upload, permission, service-worker, Push, network, account, or authentication actions were performed.

## 2026-09-05 — Audit fix 9 amendment: inventory count-syncs for the dead-function removal

The hygiene batch's dead-function removal (the legacy `addStoryTextMode`) deliberately lowered two published inventory pins, and both were re-verified and synced in this amendment: the top-level function-name inventory moves from 468 to 467 (`docs/cross-module-function-collision-contract-harness.js`, the removed declaration was the only change; the duplicate-name assertion still enforces zero collisions), and the explicit window-assignment count moves from 449 to 448 (`docs/window-assignment-surface-contract-harness.js`, because the dead function body held one `window._storyFile = null;` assignment; the `_storyFile` name itself remains established through `prevStoryMedia`'s live assignment, so the name allowlist and the missing/unexpected assertions are unchanged in strength). Both count-syncs are the repo's standard deliberate inventory-update protocol — no assertion was weakened or removed. Zero live application, database, storage, upload, permission, service-worker, Push, network, account, or authentication actions were performed.

## 2026-09-05 — Audit-fix series close-out: final verification

The owner-authorized audit-fix series is complete: fifteen bounded commits from `64cf2af` to this checkpoint, each pushed individually with the full 322-harness regression run after the push. Final verification at close: full harness suite 322/322 PASS, app-load test 10/10 PASS (463 local script references, 465 balanced script tags, 452 feature files, all 463 scripts syntax-valid, all CSS/PWA assets serving), worktree clean, local HEAD equal to `origin/Branch2`, and `origin/main` immutable at `ef418007c9b9a797488b4825be5f0c807da22369` throughout the series. Fixes 1 through 7 and 9 are applied (smart-reply wiring, comment-moderation lookup, following-count stat, notification badge, video-length picker, the staged escaping series over the existing shared `esc()` helper, the eval-to-dispatch-table refactor, and the hygiene batch); fix 8 (service-worker cache versioning) is deliberately skipped as deploy-gated per the owner's instruction. The deferred items awaiting owner decisions — the three pre-existing dead `v2` wrapper guards inherited from `origin/main`, the remaining ~60 lower-priority interpolation sites, the scaffolding-directory reorganization, and the nav-debug diagnostics — are enumerated in the handoff close-out entry. Zero live application, database, storage, upload, permission, service-worker, Push, network, account, or authentication actions were performed across the entire series.
